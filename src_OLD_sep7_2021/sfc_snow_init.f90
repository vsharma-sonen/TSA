...
! -----------------------
  ...
  REAL    (KIND=vpp) ::             &
    ...
    hm_sn      (nvec,1:ke_snow)  , &   ! height (from bottom) of snow layers (main levels)      (m)
    zm_sn      (nvec,1:ke_snow)  , &   ! depth  (from top) of snow layers (main levels)         (m)
    rho_sn     (nvec,1:ke_snow)  , &   ! density of snow layers                                 (kg/m**3)
    m_sn       (nvec,1:ke_snow)  , &   ! snow layer mass                                        (kg)
    hcap_sn    (nvec,1:ke_snow)  , &   ! snow layer heat capacity
    hcon_sn    (nvec,1:ke_snow)        ! snow layer heat conductivity
