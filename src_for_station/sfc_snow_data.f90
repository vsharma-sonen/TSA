...
! -------------------------------------------------
  ...
  INTEGER                    :: &
    ...
    n_layers    = 25     , &        ! maximum number of layers, i.e. vertical extent of fields   (-)
    n_substrate = 1                  ! number of substrate layer, i.e. layers below the snow
                                     ! NOTE: This is ke_soil!!!!!
  
    !$acc declare create (n_layers, n_substrate)


  REAL  (KIND=vpp)           ::  &

    min_height_layer = 0.0025_vpp       , &  ! minimum layer thickness
    max_height_layer = 0.02_vpp            ! maximum height/thickness of new snow layers                 (m)
  ...
  REAL (KIND=vpp), PARAMETER        ::    &
    ...
    height_meteo_values = 10.0_vpp       , & ! height of meteo values above ground soil                     (m)
...
SUBROUTINE snow_wkarr_alloc (ke_soil, n_layers, nproma, istat)

  INTEGER, INTENT(IN)  :: ke_soil, n_layers, nproma
  ...
  ALLOCATE ( hm_sn       (nproma,n_layers)  , & ! height (from bottom) of snow layers (main level)     (m)
             zm_sn       (nproma,n_layers)  , & ! depth (from top) of snow layers (main level)         (m)
             rho_sn      (nproma,n_layers)  , & ! density of snow layers (kg/m**3)
             m_sn        (nproma,n_layers)  , & ! layer mass (kg)
             hcap_sn     (nproma,n_layers)  , & ! snow layer heat capacity
             hcon_sn     (nproma,n_layers)  , & !            heat conductivity
             hdif_sn     (nproma,n_layers)  , & !            heat diffusion
             swabs_sn    (nproma,n_layers)  , & ! absorbed short wave radiation
             zm_sn_old   (nproma,n_layers)  , & ! old value of layer depth
             theta_i_old (nproma,n_layers)  , & !              volumetric ice content
             theta_w_old (nproma,n_layers)  , & !              volumetric water content
       STAT=istat)
  ...
  ALLOCATE ( tmp_sn     (n_layers)  , & ! temporary array
       STAT=istat)
