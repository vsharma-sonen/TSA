...
USE sfc_snow_data, ONLY : n_layers
...
! ALLOCATE(rh_2m_b        (nproma)     , STAT=izl); rh_2m_b        = r_init_val; ist=ist+izl
  ...
  ALLOCATE(t_sn_b         (nproma,1:n_layers), STAT=izl); t_sn_b            = r_init_val; ist=ist+izl
  ALLOCATE(t_sn_new_b     (nproma,1:n_layers), STAT=izl); t_sn_new_b        = r_init_val; ist=ist+izl

  ALLOCATE(theta_i_b      (nproma,1:n_layers), STAT=izl); theta_i_b         = r_init_val; ist=ist+izl
  ALLOCATE(theta_i_new_b  (nproma,1:n_layers), STAT=izl); theta_i_new_b     = r_init_val; ist=ist+izl

  ALLOCATE(theta_w_b      (nproma,1:n_layers), STAT=izl); theta_w_b         = r_init_val; ist=ist+izl
  ALLOCATE(theta_w_new_b  (nproma,1:n_layers), STAT=izl); theta_w_new_b     = r_init_val; ist=ist+izl

  ALLOCATE(theta_a_b      (nproma,1:n_layers), STAT=izl); theta_a_b         = r_init_val; ist=ist+izl
  ALLOCATE(theta_a_new_b  (nproma,1:n_layers), STAT=izl); theta_a_new_b     = r_init_val; ist=ist+izl

  ALLOCATE(dzm_sn_b       (nproma,1:n_layers), STAT=izl); dzm_sn_b          = r_init_val; ist=ist+izl
  ALLOCATE(dzm_sn_new_b   (nproma,1:n_layers), STAT=izl); dzm_sn_new_b      = r_init_val; ist=ist+izl
