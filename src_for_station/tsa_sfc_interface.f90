...
!    swdir_s_b       , swdifd_s_b      , swdifu_s_b      , lwd_s_b         , &
!    lwu_s_b                                                               , &
    ! output variables                                                   
    ...
    hn_sn_new_b
!VS >
USE data_fields,      ONLY : p0, pp, depth_lk, qmomflux, umfl_s, vmfl_s,    &
                             fr_lake, llandmask, fr_land, isoiltyp, prh_gsp,&
                             !t_so_sijk    , w_so_sijk    , w_so_ice_sijk,   &
                             !t_ice_sijk   , h_ice_sijk   , t_mnw_lk_sijk,   &
                             !t_wml_lk_sijk, t_bot_lk_sijk, h_ml_lk_sijk ,   &
                             !c_t_lk_sijk  , t_snow_sijk  , h_snow_sijk  ,   &
                             !t_b1_lk_sijk , h_b1_lk_sijk , t_s_sijk     ,   &
!VS <
                             rlat, rlon
!VS >
...
!!DL   IF (llake) THEN
!!DL     CALL flake_wkarr_alloc (nproma, istat)
!!DL   ENDIF
!!DL   IF (lseaice) THEN
!!DL     CALL seaice_wkarr_alloc (nproma, istat)
!!DL   ENDIF
!VS <
  IF (lsnow) then
    CALL snow_wkarr_alloc (ke_soil, n_layers, nproma, istat)
  endif
!VS >
...
!VS <
  REAL (KIND=vpp), DIMENSION(nproma,1:n_layers)             ::                            &
      t_sn_now_t     , t_sn_new_t                                                       , &
      theta_i_now_t  , theta_i_new_t                                                    , &
      theta_w_now_t  , theta_w_new_t                                                    , &
      theta_a_now_t  , theta_a_new_t                                                    , &
...
!VS <
  !$acc present (t_sn_now_t         , t_sn_b                                       ) &
  !$acc present (theta_i_now_t      , theta_i_b                                    ) &
  !$acc present (theta_w_now_t      , theta_w_b                                    ) &
  !$acc present (theta_a_now_t      , theta_a_b                                    ) &
  !$acc present (dzm_sn_now_t       , dzm_sn_b                                     ) &
  !$acc present (hn_sn_now_t        , hn_sn_b                                      ) &
  !$acc present (top_sn_now_t       , top_sn_b                                     ) &
!VS >
...
!VS >
  !$acc present (t_sn_new_t      , t_sn_new_b                                      ) &
  !$acc present (theta_i_new_t   , theta_i_new_b                                   ) &
  !$acc present (theta_w_new_t   , theta_w_new_b                                   ) &
  !$acc present (theta_a_new_t   , theta_a_new_b                                   ) &
  !$acc present (dzm_sn_new_t    , dzm_sn_new_b                                    ) &
  !$acc present (hn_sn_new_t     , hn_sn_new_b                                     ) &
  !$acc present (top_sn_new_t    , top_sn_new_b                                    ) &
!VS <
...
!VS <
  ALLOCATE ( t_sn_now_t         (nproma,1:n_layers) ,STAT=izl ); t_sn_now_t    = 0.0_wp; ist=ist+izl
  ALLOCATE ( t_sn_new_t         (nproma,1:n_layers) ,STAT=izl ); t_sn_new_t    = 0.0_wp; ist=ist+izl
  ALLOCATE ( theta_i_now_t      (nproma,1:n_layers) ,STAT=izl ); theta_i_now_t = 0.0_wp; ist=ist+izl
  ALLOCATE ( theta_i_new_t      (nproma,1:n_layers) ,STAT=izl ); theta_i_new_t = 0.0_wp; ist=ist+izl
  ALLOCATE ( theta_w_now_t      (nproma,1:n_layers) ,STAT=izl ); theta_w_now_t = 0.0_wp; ist=ist+izl
  ALLOCATE ( theta_w_new_t      (nproma,1:n_layers) ,STAT=izl ); theta_w_new_t = 0.0_wp; ist=ist+izl
  ALLOCATE ( theta_a_now_t      (nproma,1:n_layers) ,STAT=izl ); theta_a_now_t = 0.0_wp; ist=ist+izl
  ALLOCATE ( theta_a_new_t      (nproma,1:n_layers) ,STAT=izl ); theta_a_new_t = 0.0_wp; ist=ist+izl
  ALLOCATE ( dzm_sn_now_t       (nproma,1:n_layers) ,STAT=izl ); dzm_sn_now_t  = 0.0_wp; ist=ist+izl
  ALLOCATE ( dzm_sn_new_t       (nproma,1:n_layers) ,STAT=izl ); dzm_sn_new_t  = 0.0_wp; ist=ist+izl
