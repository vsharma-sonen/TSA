...
!  added some code to make sfc_terra work with snowpolino
! Code Description:
! Language: Fortran 90.
...
! -------------------------------------------
    ...
    r_d_wp     => r_d,     & ! gas constant for dry air
    rdv_wp     => rdv,     & ! r_d / r_v
    o_m_rdv_wp => o_m_rdv, & ! 1 - r_d/r_v
    rvd_m_o_wp => rvd_m_o, & ! r_v/r_d -1
    cp_d_wp    => cp_d,    & ! specific heat of dry air at constant pressure
    rdocp_wp   => rdocp,   & ! r_d / cp_d
    lh_v_wp    => lh_v,    & ! latent heat of vapourization
    lh_f_wp    => lh_f,    & ! latent heat of fusion
    lh_s_wp    => lh_s,    & ! latent heat of sublimation
    g_wp       => g,       & ! acceleration due to gravity
    sigma_wp   => sigma,   & ! Boltzmann-constant

! 2. constants for parametrizations
! ---------------------------------
    b1_wp      => b1,      & ! variables for computing the saturation vapour pressure
    b2w_wp     => b2w,     & ! over water (w) and ice (i)
    b2i_wp     => b2i,     & !               -- " --
    b3_wp      => b3,      & !               -- " --
    b4w_wp     => b4w,     & !               -- " --
    b4i_wp     => b4i,     & !               -- " --
    rho_w_wp   => rho_w      ! density of liquid water
...
!
USE mo_kind,               ONLY: vpp
USE mo_math_constants    , ONLY: pi_wp => pi
!
USE mo_physical_constants, ONLY: t0_melt_wp => tmelt   , & ! absolute zero for temperature
                                 r_d_wp     => rd      , & ! gas constant for dry air
                                 rvd_m_o_wp => vtmpc1  , & ! r_v/r_d - 1
                                 o_m_rdv_wp => o_m_rdw , & ! 1 - r_d/r_v
                                 rdv_wp     => rdw     , & ! r_d / r_v
                                 lh_v_wp    => alv     , & ! latent heat of vapourization
                                 lh_s_wp    => als     , & ! latent heat of sublimation
                                 lh_f_wp    => alf     , & ! latent heat of fusion
                                 cp_d_wp    => cpd     , & ! specific heat of dry air at constant press
                                 g_wp       => grav    , & ! acceleration due to gravity
                                 b3_wp      => t3      , & ! triple point of water at 611hPa
                                 sigma_wp   => stbo    , & ! Boltzmann-constant
                                 rho_w_wp   => rhoh2o  , & ! density of liquid water (kg/m^3)
                                 rdocp_wp   => rd_o_cpd    ! r_d / cp_d
!
USE mo_convect_tables,     ONLY: b1_wp      => c1es    , & !! constants for computing the sat. vapour
                                 b2w_wp     => c3les   , & !! pressure over water (l) and ice (i)
                                 b2i_wp     => c3ies   , & !!               -- " --
                                 b4w_wp     => c4les   , & !!               -- " --
                                 b4i_wp     => c4ies   , & !!               -- " --
                                 b234w_wp   => c5les       !!               -- " --
...
!                 w_isa                ! same, multiplied by fr_paved                    --
  ...
  REAL    (KIND = vpp), DIMENSION(nvec,0:ke_snow), INTENT(INOUT) :: &
                  t_snow_mult_now      ! temperature of the snow-surface               (  K  )
  REAL    (KIND = vpp), DIMENSION(nvec,0:ke_snow), INTENT(OUT) :: &
                  t_snow_mult_new      ! temperature of the snow-surface               (  K  )

  REAL    (KIND = vpp), DIMENSION(nvec,ke_snow), INTENT(INOUT) :: &
                  rho_snow_mult_now, & ! snow density                                  (kg/m**3)
                  wliq_snow_now    , & ! liquid water content in the snow              (m H2O)
                  wtot_snow_now    , & ! total (liquid + solid) water content of snow  (m H2O)
                  dzh_snow_now         ! layer thickness between half levels in snow   (  m  )

  REAL    (KIND = vpp), DIMENSION(nvec,ke_snow), INTENT(OUT) :: &
                  rho_snow_mult_new, & ! snow density                                  (kg/m**3)
                  wliq_snow_new    , & ! liquid water content in the snow              (m H2O)
                  wtot_snow_new    , & ! total (liquid + solid) water content of snow  (m H2O)
                  dzh_snow_new         ! layer thickness between half levels in snow   (  m  )
...
!US why +1  REAL    (KIND = vpp), DIMENSION(nvec,ke_soil+1), INTENT(OUT) :: &
...
! ----------------
  ...
  REAL(KIND=vpp), PARAMETER ::  &
    zalfa      =  1.0_vpp, & ! degree of impliciteness (1: full implicit,
                            !    (0.5: Cranck-Nicholson)
    T_ref_ice  =  0.1_vpp, & !degC Soil ice parameterization
    T_star_ice =  0.01_vpp,& !degC according to K. Schaefer and Jafarov, E.,2016, doi:10.5194/bg-13-1991-2016
    b_clay     = -0.3_vpp, &
    b_silt     = -0.5_vpp, &
    b_sand     = -0.9_vpp, &
    b_org      = -1.0_vpp, &
    rho_i      = 910.0_vpp   ! density of solid ice (soil model)  (kg/m**3)
...
! -------------
  ...
  REAL(KIND=vpp)  ::  &
    ...
    zpsi0=0.01_vpp  , & ! air entry potential at water saturation (m)
  ...
  REAL    (KIND=vpp) ::  &
    ...
    zch_soil  = 1.4E06_vpp , & ! approximate average heat capacity of soil (J/m**3 K)
    zlim_dtdt = 2.5_vpp    , & ! maximum allowed temperature increment (K) in
...
! -------------------------
  ...
  REAL    (KIND=vpp) :: ln_2, ln_3, ln_10, ln_006
...
mvid =   8
  ...
  r_d     = REAL(r_d_wp    , vpp)
  rdv     = REAL(rdv_wp    , vpp)
  o_m_rdv = REAL(o_m_rdv_wp, vpp)
  rvd_m_o = REAL(rvd_m_o_wp, vpp)
  cp_d    = REAL(cp_d_wp   , vpp)
  rdocp   = REAL(rdocp_wp  , vpp)
  lh_v    = REAL(lh_v_wp   , vpp)
  lh_f    = REAL(lh_f_wp   , vpp)
  lh_s    = REAL(lh_s_wp   , vpp)
  g       = REAL(g_wp      , vpp)
  sigma   = REAL(sigma_wp  , vpp)
  b1      = REAL(b1_wp     , vpp)
  b2w     = REAL(b2w_wp    , vpp)
  b2i     = REAL(b2i_wp    , vpp)
  b3      = REAL(b3_wp     , vpp)
...
! Computation of derived constants
  ...
  zdwi_scal = zdt/1800.0_vpp  ! time scale parameter for freezing/melting soil water
...
!                                                    doi:10.5194/bg-13-1991-2016
  t_zw_up  = 270.15_vpp     ! temp -3 degC
  t_zw_low = 233.15_vpp     ! temp -40 degC
...
! Prepare basic surface properties (for land-points only)
  ...
  DO i = ivstart, ivend
    ...
    IF (mstyp == 1) h_snow(i) = MAX(1.0_vpp, h_snow(i))
    zdw       (i,:) = cdw0  (mstyp)
    zdw1      (i,:) = cdw1  (mstyp)
    zkw       (i,:) = ckw0  (mstyp)
    zkwm      (i,:) = ckw0  (mstyp)
    ...
    zbwt      (i)   = MAX(0.001_vpp,rootdp(i))    ! Artificial minimum value for root depth
    zroota    (i)   = 3.0_vpp/zbwt(i)             ! root density profile parameter (1/m)
  ...
  DO i = ivstart, ivend
    ...
    zsiltf(i)   = 100.0_vpp -csandf(mstyp)-cclayf(mstyp) ! Residuum of sand and clay
    zpsis (i)   = -zpsi0 * EXP(ln_10*(1.88_vpp-0.013_vpp*zsandf(i)))
    zb_por(i)   = 2.91_vpp + 0.159_vpp*zclayf(i)
    zedb  (i)   = 1.0_vpp/zb_por(i)
    ...
    !  J. Helmert: Soil ice parameterization according to K. Schaefer and Jafarov, E.,2016,
    !                                                    doi:10.5194/bg-13-1991-2016
    zw_m_soil(i) = 0.01_vpp*(zsandf(i)*zd1 + zclayf(i)*zd2 + zsiltf(i)*zd3)
    zw_m_org(i) = zd4
  ENDDO
  !$acc end parallel
  ...
  IF (itype_interception == 2) THEN
    DO i = ivstart, ivend
      ...
      zkw0   (i) = zkw   (i,1)*EXP(2.0_vpp*rootdp(i))
    END DO
  ENDIF
  ...
  DO kso = 1, ke_soil
    DO i = ivstart, ivend
      !fc=2 1/m Exponential Ksat-profile decay parameter,see Decharme et al. (2006)
      zkw   (i,kso) = zkw   (i,kso)*EXP(-2.0_vpp*(zzhls(kso)-rootdp(i)))
      zkwm  (i,kso) = zkwm  (i,kso)*EXP(-2.0_vpp*(zmls(kso)-rootdp(i)))
      ...
      IF (zmls(kso) < rootdp(i)) THEN
        ...
        zw_m_low(i,kso) = zporv(i,kso)*zdzhs(kso)*(zzz*zw_m_org(i) + (1.0_vpp-zzz)*zw_m_soil(i))
      ELSE
  ...
  DO i = ivstart, ivend
    ...
    zs1    (i)            = 0.0_vpp
    zwroot (i)            = 0.0_vpp
    zdwidt (i)            = 0.0_vpp         ! Initialisation of all
    zdwsndt(i)            = 0.0_vpp         ! evaporation quantities
    zesoil (i)            = 0.0_vpp         ! to be equal to zero
    zrr    (i)            = 0.0_vpp         ! in first part formation of dew
    zrs    (i)            = 0.0_vpp         ! in first part formation of rime
...
!   ENDIF
  ...
  DO kso   = 1, ke_soil
    DO i = ivstart, ivend
      ...
      ztrang  (i,kso)     = 0.0_vpp
      zropartw(i,kso)     = 0.0_vpp
    ENDDO
  ENDDO
  !$acc end parallel
  ...
  znull  = zzhls(1)
  DO kso = 1, ke_soil
    IF (zmls(kso).le.0.1_vpp) THEN
      z1      = zzhls(kso)
      k10cm   = kso
    END IF
    IF (zmls(kso).le.1.0_vpp) THEN
      znull   = zzhls(kso)
      k100cm  = kso
    END IF
  END DO
...
#endif
        w_so_now(i,kso)         = 0.0_vpp
        w_so_ice_now(i,kso)     = 0.0_vpp
...
!CDIR NODEP,VOVERTAKE,VOB
  ...
  IF (itype_heatcond >= 2) THEN
...
#ifndef _OPENACC
    zlamli    = LOG(0.57_vpp)       ! LOG(thermal conductivity of water)
    zlamic    = LOG(2.2_vpp)        ! LOG(thermal conductivity of ice)
    zlamq     = LOG(7.7_vpp)        ! LOG(thermal conductivity of quartz)
    ln_2      = LOG(2.0_vpp)
    ln_3      = LOG(3.0_vpp)
    ln_006    = LOG(0.06_vpp)
    zlamspeat = LOG(0.25_vpp)! AYu lambdas for peat (Lawrence & Slater, 2008)
#endif

    zlamdrypeat =   0.05_vpp ! AYu lambdas for peat (Lawrence & Slater, 2008)
    zkeai       = 0.6116_vpp ! AYu 4 below for the new Kersten number parametrization
    ...
    IF (itype_evsl == 4) THEN
      zlamdry_c1 = 437.0_vpp
      zlamdry_c2 = 0.901_vpp
    ELSE
      zlamdry_c1 = 64.7_vpp
      zlamdry_c2 = 0.947_vpp
    ENDIF


    !$acc parallel async
    !$acc loop gang vector collapse(2) private(zthetas, zthliq, rsandf, zlams) &
    ...
    DO kso = 1, ke_soil+1
      DO i = ivstart, ivend
        ...
        rsandf = zsandf(i)/100.0_vpp                     ! quartz content

        if (rsandf >= 0.2_vpp)  zlam0 = ln_2      ! LOG(thermal conductivity non-quartz)
        if (rsandf <  0.2_vpp)  zlam0 = ln_3
  ...
  ! saturated thermal conductivity
        ...
        ELSE
          zlams = zlamq*rsandf + zlam0*(1.0_vpp-rsandf)  ! LOG(solids thermal conductivity)
        ENDIF

        zlamsat = EXP(zlams*(1.0_vpp-zthetas) + zlamic*(zthetas-zthliq) + zthliq*zlamli)

  ! dry thermal conductivity

        zrhod   = 2700.0_vpp*(1.0_vpp-zthetas)       ! dry density
        zlamdry_soil = ( 0.135_vpp*zrhod + zlamdry_c1 )                     &
                / ( 2700.0_vpp - zlamdry_c2*zrhod )
        ! missing: crushed rock formulation for dry thermal conductivity (see PL98)
  ! Scale zlamdry with organic fraction
        IF(zmls(kso) < rootdp(i)) THEN
          zzz = plcov(i)*(rootdp(i)-zmls(kso))/rootdp(i)
          zlamdry = EXP(LOG(zlamdry_soil)*(1.0_vpp-zzz)+ln_006*zzz) ! Chadburn et al.,2015, Dankers et al., 2011
        ELSE
          zzz = 0.0_vpp
  ...
  ! Kersten number
        ...
        zsri = MIN(1.0_vpp, w_so_now(i,kso)/zdzhs(kso) / zthetas) ! degree of saturation
        ...
        ELSE                                                         ! unfrozen
          ...
          IF ( soiltyp_subs(i) == 3 .or. soiltyp_subs(i) == 4 ) THEN ! coarse soil
            IF ( zsri >= 0.05_vpp ) THEN
              zKe = 0.7_vpp*LOG(zsri)/ln_10 + 1.0_vpp
            ENDIF
          ELSE                                                       ! fine soil (other)
            IF ( zsri >= 0.1_vpp ) THEN
              zKe = LOG(zsri)/ln_10 + 1.0_vpp
            ENDIF
          ENDIF
          IF (soiltyp_subs(i) == 8 .AND. itype_mire == 1) zKe = MAX(0.0_vpp, zKe) ! AYu can be more than 1 for the peat
        ENDIF
        zKe = MAX(0.0_vpp, (MIN(1.0_vpp, zKe)) )

  ! thermal conductivity

        ! tuning factor to indirectly account for the impact of vegetation, which does not depend on soil moisture
        IF(itype_heatcond == 3 .AND. zmls(kso) < 0.075_vpp) THEN
          zxx = 12.5_vpp*(0.075_vpp-zmls(kso))*zzz
        ELSE
          zxx = 0.0_vpp
        ENDIF
        hzalam(i,kso) = (zKe*(zlamsat - zlamdry) + zlamdry)*(1.0_vpp-zxx) + zxx*0.06_vpp
        ...
        IF (tsnred(i) < -1.0_vpp .AND. z0(i) >= 0.2_vpp) THEN
          zxx = MAX(0.0_vpp,2.0_vpp - ABS(tsnred(i)))
          hzalam(i,kso) = zxx*hzalam(i,kso) + (1.0_vpp-zxx)*0.06_vpp
        ENDIF
#endif
      ENDDO
    ENDDO
...
!   DO kso = 1, ke_soil
!     zalpha_uf  = MAX (0.0_vpp, MIN(zmls(kso)/c_uf_h ,1.0_vpp))
!     zalpha_lnd = MAX (0.0_vpp, MIN(zmls(kso)/c_lnd_h,1.0_vpp))

!     DO i = ivstart, ivend
!       zalam(i,kso) = sa_uf(i) * c_ala_bm     * ( ai_uf(i)*(1.0_vpp - zalpha_uf ) + zalpha_uf ) + &
!             (1.0_vpp-sa_uf(i)) * zalam(i,kso) *    ( c_lnd*(1.0_vpp - zalpha_lnd) + zalpha_lnd)
!     ENDDO
!   ENDDO
! END IF
...
! Initialisations and conversion of tch to tmch
  ...
  !$acc parallel async
  !$acc loop gang worker vector private(zuv, ztvs, zplow, zdT_snow)
  DO i = ivstart, ivend
    zuv        = SQRT ( u(i)**2 + v(i)**2 )
    ztvs       = t_g (i)*(1.0_vpp + rvd_m_o*qv_s(i))
...
#ifdef __ICON__
    zxx = MIN(500.0_vpp,200.0_vpp+0.5_vpp*ABS(zradfl(i)))
#endif
#ifdef __COSMO__
    zxx = 500.0_vpp
#endif
    IF (zshfl(i)*zlhfl(i) >= 0.0_vpp) THEN
    ...
    zch_snow(i)=MIN(w_snow_now(i),1.5_vpp*rho_snow_now(i)/rho_w)*rho_w*chc_i   ! (J/(m**2 K))
    ...
    IF (zeb1(i)<0.0_vpp .AND. zthfl(i)<0.0_vpp) THEN
    ...
    ELSEIF (zeb1(i)>0.0_vpp .AND. zthfl(i)>0.0_vpp) THEN
      ...
      IF (lmulti_snow) THEN
        zdT_snow=MIN(0.0_vpp, t0_melt-0.5_vpp*(t_snow_mult_now(i,1)+t_so_now(i,1)))
      ELSE
        zdT_snow=MIN(0.0_vpp,t0_melt-0.5_vpp*(t_snow_now(i)+t_so_now(i,1)))
      ENDIF
    ...
    ztchv_max(i) = MIN(ztchv_max(i),(4.0_vpp*zlim_dtdt*(zch_soil*zdzhs(1)+zch_snow(i))/zdt &
                  +ABS(zradfl(i)))/ABS(zthfl(i))*tch(i)*zuv)

    ztchv(i)    = tch(i)*zuv  ! transfer coefficient * velocity

    LIM: IF (ztchv(i) > ztchv_max(i)) THEN
      tch(i)=ztchv_max(i)/MAX(zuv,1.E-06_vpp)
  ...
  !$acc parallel async
  !$acc loop gang worker vector private(zsnow_rate, zdsn_new, zdsn_old, ztau_snow, zuv)
  DO i = ivstart, ivend
    IF (w_snow_now(i) <=0.0_vpp) THEN
    ...
    ELSE
      ...
      ztau_snow = 86400.0_vpp*MIN(28.0_vpp,2.0_vpp+1.733_vpp*(t0_melt-MIN(t0_melt,t_snow_now(i))))
      ...
      zuv = MIN(300.0_vpp, u_10m(i)**2 + v_10m(i)**2 + 12.0_vpp )
      ztau_snow = MIN(ztau_snow,MAX(86400.0_vpp,2.e8_vpp*MAX(0.05_vpp,h_snow_gp(i))/zuv))

      ! decay rate for fresh snow including contribution by rain (full aging after 10 mm of rain)
      zdsn_old   = zdt/ztau_snow + zdt*zrain_rate*0.1_vpp
...
!------------------------------------------------------------------------------
  ...
  IF (lmulti_snow) THEN
    ...
    DO i = ivstart, ivend
      ...
      zhm_snow(i,1) = (-h_snow_now(i) + zhh_snow(i,1))/2.0_vpp

      zdzh_snow(i,1) = dzh_snow_now(i,1)
      zextinct (i,1) = 0.13_vpp*rho_snow_mult_now(i,1)+3.4_vpp
    ...
    DO  ksn = 2,ke_snow
      ...
      DO i = ivstart, ivend
        ...
        zhm_snow   (i,ksn) = (zhh_snow(i,ksn) + zhh_snow(i,ksn-1))/2.0_vpp

        zdzh_snow  (i,ksn) = dzh_snow_now(i,ksn)
        zextinct   (i,ksn) = 0.13_vpp*rho_snow_mult_now(i,ksn)+3.4_vpp
...
!US as long as itype_canopy is not really implemented in ICON
    ...
    IF (itype_canopy == 1) THEN
      ztfunc  (i) = MAX(0.0_vpp,1.0_vpp - MAX(0.0_vpp,0.5_vpp*(zts(i)-t0_melt)))
    ELSE IF (itype_canopy == 2) THEN
      ztfunc  (i) = MAX(0.0_vpp,1.0_vpp - MAX(0.0_vpp,0.5_vpp*(ztsk(i)-t0_melt)))
    END IF
    ...
    zrhoch(i)   = ztmch(i)*(1.0_vpp/g) + eps_soil

    ! saturation specific humidity for t_s and t_snow and first derivative
    IF (itype_canopy == 1) THEN
      z2iw      = zts_pm(i)*b2w + (1.0_vpp - zts_pm(i))*b2i
      z4iw      = zts_pm(i)*b4w + (1.0_vpp - zts_pm(i))*b4i
...
#ifdef _OPENACC
! YB: this prevents PGI "Scalar last value needed..."
    ELSE
      zqs = 0.0_vpp  ! this value is arbitrary and will never be set
#endif
    END IF
    zdqs        = zqvlow - zqs
    IF (ABS(zdqs).LT.0.01_vpp*eps_soil) zdqs = 0.0_vpp
    z2iw        = ztsnow_pm(i)*b2w + (1.0_vpp - ztsnow_pm(i))*b2i
    z4iw        = ztsnow_pm(i)*b4w + (1.0_vpp - ztsnow_pm(i))*b4i
    z234iw      = z2iw*(b3 - z4iw)
    zqsnow    = zsf_qsat(zsf_psat_iw(ztsnow(i)-MAX(0.0_vpp,tsnred(i)),z2iw,z4iw), ps(i))

    zdqvtsnow(i)= zsf_dqvdt_iw(ztsnow(i), zqsnow, z4iw,z234iw)
    zdqsnow     = zqvlow - zqsnow
    IF (ABS(zdqsnow).LT.0.01_vpp*eps_soil) zdqsnow = 0.0_vpp

    ! potential evaporation at T_snow and Ts
    zep_snow(i) = (1.0_vpp-ztsnow_pm(i))* tfv(i)*zrhoch(i)*zdqsnow
...
!------------------------------------------------------------------------------
  ...
  DO i = ivstart, ivend
     ...
     !em        zrss = MAX( 0.01_vpp, MIN(1.0_vpp,zwsnow(i)/cf_snow) )
     zzz  = MAX( 0.25_vpp*cf_w,0.4_vpp*cwimax_ml*MAX(2.5_vpp*plcov(i),tai(i)) )
     zrww = MAX( 0.01_vpp, 1.0_vpp - EXP(MAX( -5.0_vpp, - zwin(i)/zzz) ) )
     !em        zf_snow(i) = zrss*zsf_heav(zwsnow(i) - eps_soil)

     IF (itype_interception == 1) THEN
       zf_wi  (i) = zrww*zsf_heav(zwin  (i) - 1.0E-4_vpp*eps_soil)
     ELSE IF (itype_interception == 2) THEN
     ...
     zf_snow_lim(i) = MAX(0.01_vpp,0.1_vpp*freshsnow(i),zf_snow(i))
     ...
     zdz_snow_fl(i) = MIN(1.5_vpp, zdz_snow(i))
...
!------------------------------------------------------------------------------
  ...
  DO i = ivstart, ivend
    ...
    zzz = (1.0_vpp + 0.5_vpp*ztfunc(i))/3.0_vpp
    zdwidt(i) = zsf_heav(-zep_s(i)) * MAX(-zrhwddt*zwin(i),              &
      zzz*zf_wi(i)*zep_s(i), -MAX(300.0_vpp,0.75_vpp*zradfl(i))/lh_v)
    ...
    IF (itype_canopy == 1) THEN
      ...
      zrs(i)=zsf_heav(zep_snow(i))*zep_snow(i)*(1.0_vpp-zts_pm(i))
    ELSE IF (itype_canopy == 2) THEN
      zrr(i)=zsf_heav(zep_s   (i))*zep_s   (i)*        ztsk_pm(i)
      zrs(i)=zsf_heav(zep_snow(i))*zep_snow(i)*(1.0_vpp-ztsk_pm(i))
    END IF
  ENDDO
  !$acc end parallel
  ...
  IF (itype_interception == 2) THEN
    ...
    !$acc parallel async
    !$acc loop gang vector private(zpercmax)
    DO i = ivstart, ivend
      zwimax(i) = MAX(1.E-6_vpp,4.E-4_vpp * sai(i)) ! Security min. value 1E-6 m
      zpercmax = 2.E-3_vpp
      zfd_wi(i)=MIN(1.0_vpp,MAX(0.0_vpp, EXP((2.0_vpp/3.0_vpp)*LOG(zwin(i)/zwimax(i))) ))
      zf_pd(i) = MIN(1.0_vpp,MAX(0.0_vpp, EXP((2.0_vpp/3.0_vpp)*LOG(zwpn(i)/zpercmax)) ))

      zewi(i)=zsf_heav(-zep_s(i)) * zf_wi(i)*zfd_wi(i)*zep_s(i) ! canopy covered part
      zepd(i)=zsf_heav(-zep_s(i)) * (1.0_vpp - zf_wi(i))*zf_pd(i)*zep_s(i) ! bare soil part
      ...
      IF (itype_canopy == 1) THEN
        ...
        zrrs(i) = zsf_heav(zep_snow(i))*zep_snow(i)*(1.0_vpp-zts_pm(i))
      ELSE IF (itype_canopy == 2) THEN
        zdrr(i) = zsf_heav(zep_s   (i))*zep_s   (i)*        ztsk_pm(i)
        zrrs(i) = zsf_heav(zep_snow(i))*zep_snow(i)*(1.0_vpp-ztsk_pm(i))
      END IF
    END DO
    !$acc end parallel
  ...
  IF (itype_evsl.EQ.2) THEN
    ...
    DO i = ivstart, ivend
      IF (zep_s(i) < 0.0_vpp) THEN   ! upwards directed potential evaporation
        zsnull(i) = zsnull(i)/(znull*zporv(i,1))

        ! Treatment of ice (m_styp=1) and rocks (m_styp=2)
        zice   = zsf_heav(1.5_vpp - REAL(m_styp(i),vpp)) ! 1 only for ice
        ...
        IF (m_styp(i).ge.3) THEN ! Computations not for ice and rocks
          ...
          zbf1   = 5.5_vpp - 0.8_vpp* zbedi(i)*                &
                  (1.0_vpp + 0.1_vpp*(zbedi(i) - 4.0_vpp)*  &
                   clgk0(m_styp(i)) )
          zbf2   = (zbedi(i) - 3.7_vpp + 5.0_vpp/zbedi(i))/  &
                  (5.0_vpp + zbedi(i))
          zdmax  = zbedi(i)*cfinull*zk0di(i)/crhowm
          zs1(i)  = zs1(i)/(z1*zporv(i,1))
          zd     = 1.02_vpp*zdmax*EXP( (zbedi(i)+2.0_vpp)*LOG(zs1(i)) ) * &
                                     EXP( zbf1*LOG(zsnull(i)/zs1(i)) )
          zck    = (1.0_vpp + 1550.0_vpp*cdmin/zdmax)*zbf2
        ...
        IF (soiltyp_subs(i) == 8 .AND. itype_mire == 1) THEN      ! AYu mire block
          zbeta           = 0.6_vpp
          zsfc_frac_bs(i) = 1.0_vpp
        ENDIF

        ! consideration of plant or snow/water cover
        IF (itype_interception == 1) THEN
          zesoil(i) = zevap*zbeta*zep_s(i)        & ! evaporation
          !!!              *(1.0_vpp - zf_wi  (i)) & ! not water covered
                           *(1.0_vpp - zf_snow(i)) & ! not snow covered
                           * zsfc_frac_bs(i)        ! relative source surface
                                                    ! of the bare soil

        ELSE IF (itype_interception == 2) THEN
          zesoil(i) = zevap*zbeta*zep_s(i)       & ! evaporation
                          *(1.0_vpp - plcov(i))   & ! plant cover weighting
                          *(1.0_vpp - zf_snow(i)) & ! not snow covered
                          *(1.0_vpp - zf_pd(i))     ! not pond covered
        END IF ! interception

!       IF (lterra_urb .AND. ((itype_eisa == 1).OR. (itype_eisa == 2))) THEN
!         ! HW: evaporation of water stored on imperivous ground
!         !zesoil(i) = zesoil(i)*(1.0_vpp - fr_paved(i))
!         IF (itype_eisa == 2) THEN
!           zeisa(i) = zep_s(i) *(1.0_vpp - zf_snow(i)) & ! not snow covered
!                     * fr_paved(i)*c_isa_delt * (w_imp(i)/c_isa_wmax)**0.6667_vpp
...
#endif
          END IF
          zevapor= MIN(0.0_vpp,zrhoch(i)*(qv(i)-zalpha*zqs))
  ...
  IF (itype_evsl.EQ.4) THEN   ! Resistance version
    ...
    DO i = ivstart, ivend
      IF (zep_s(i) < 0.0_vpp) THEN   ! upwards directed potential evaporation
        ...
        IF (m_styp(i).GE.3) THEN ! Computations not for ice and rocks
           zalpha = MAX( 0.0_vpp, MIN( 1.0_vpp,                     &
                    (zw_fr(i,1) - zadp(i,1)) / (zfcap(i,1) - zadp(i,1)) ) )
           zalpha = 50.0_vpp / (zalpha + eps_soil)
           zbeta  = 1.0_vpp                                &
                  / (1.0_vpp + zrhoch(i)*zalpha/zrho_atm(i))
        END IF ! Computations not for ice and rocks
...
!       ENDIF
  ...
  BATS: IF (itype_trvg == 2 .OR. itype_trvg == 3) THEN   ! BATS version
    ...
    IF (itype_root == 2) THEN
      ...
      !$acc loop gang vector
      DO i = ivstart, ivend
        zrootdz_int (i)= 0.0_vpp   !initialize the root density profile integral
        zwrootdz_int(i)= 0.0_vpp   !initialize the root water content integral
      END DO
      !$acc end parallel
...
!CDIR NODEP,VOVERTAKE,VOB
        ...
        DO i = ivstart, ivend
          IF (soiltyp_subs(i) >= 3 .AND. zep_s(i) < 0.0_vpp) THEN
            ...
            zwrootdz(i,kso)=zrootdz*MAX(zpwp(i,kso),zw_fr(i,kso)-10.0_vpp*w_so_ice_now(i,kso)/zdzhs(kso))
        ...
        DO i = ivstart, ivend
          IF (soiltyp_subs(i) >= 3 .AND. zep_s(i) < 0.0_vpp) THEN
             ...
             ! upwards directed potential evaporation
              zropart  = MIN ( zdzhs(kso), MAX(0.0_vpp,                     &
                               zbwt(i) - (zmls(kso) - 0.5_vpp*zdzhs(kso))))
...
!CDIR NODEP,VOVERTAKE,VOB
    !$acc parallel async
    !$acc loop gang worker vector private(i, zuv, zcatm, zustar, zrla, zpar, zf_wat) &
    !$acc private(zf_tem, z2iw, z4iw, zepsat, zepke, zf_sat, zedrstom)               &
    !$acc private(zrstom, zrveg, zxx, zzz)
    DO i = ivstart, ivend
      IF (soiltyp_subs(i) >= 3 .AND. zep_s(i) < 0.0_vpp) THEN
        zxx = 0.0_vpp ! never used, this prevents PGI "Scalar last value needed..."
#endif
        ! upwards directed potential evaporation
        zuv        = SQRT (u(i)**2 + v(i)**2 )
        ...
        SELECT CASE ( icant )
          CASE (1)   ! Louis-transfer-scheme: additional laminar canopy resistance
            ...
            zrla   = 1.0_vpp/MAX(cdash*SQRT(zustar),eps_div)
          ...
          CASE (3)   ! EDMF: additional laminar canopy resistance
            zrla   = 1.0_vpp/MAX(tch(i)*zuv,eps_div)
        END SELECT
        ...
        IF (itype_trvg == 3) THEN
          ...
          zxx      = 0.75_vpp*MAX(eps_soil,ABS(plevap(i)))/MAX(0.2_vpp,plcov(i))
          zzz      = MIN(3.0_vpp,MAX(1.0_vpp,zxx))
          ! stronger limitation for non-forest vegetation classes
          IF (z0(i) <= 0.4_vpp) zzz = MIN(2.0_vpp, zzz)
        ELSE
          zzz = 1.0_vpp
        ENDIF
        zpar     = pabs(i)/(cparcrit*zzz)   ! normalized PAR
        zf_rad(i)= MAX(0.0_vpp,MIN(1.0_vpp,zpar))
        ztlpmwp(i) = (zfcap(i,1) - zpwp(i,1))*(0.81_vpp +       &
           0.121_vpp*ATAN(-86400.0_vpp*zep_s(i) - 4.75_vpp))

        ! Soil water function
        IF (itype_root == 2) THEN
          zf_wat = MAX(0.0_vpp,MIN(1.0_vpp,(zwrootdz_int(i) - zpwp(i,1))/ztlpmwp(i)))
        ELSE
          zf_wat = MAX(0.0_vpp,MIN(1.0_vpp,(zwroot(i) - zpwp(i,1))/ztlpmwp(i)))
        ENDIF

        ! Temperature function
        ! T at lowest model level used (approximation of leaf height)
        zf_tem     = MAX(0.0_vpp,MIN(1.0_vpp,4.0_vpp*     &
        ...
        ! zf_sat     = MAX(0.0_vpp,MIN(1.0_vpp,1.0_vpp - (zepsat - zepke)/csatdef))
...
!CDIR NODEP,VOVERTAKE,VOB
        DO ic=1,icount_soil
          ...
          IF (zep_s(i) < 0.0_vpp) THEN
#else
        !$acc loop gang worker vector private(ztrabpf, ztrfr, zrootfc)
        DO i = ivstart, ivend
          IF (soiltyp_subs(i) >= 3 .AND. zep_s(i) < 0.0_vpp) THEN
#endif
            ...
            IF(zw_fr(i,kso)+ztrang(i,kso)*zdtdrhw/zdzhs(kso) < zpwp(i,kso)) &
              ztrang(i,kso) = MIN(0.0_vpp,(zpwp(i,kso)-zw_fr(i,kso))*zdzhs(kso)/zdtdrhw)
...
!CDIR NODEP,VOVERTAKE,VOB
        ...
        DO i = ivstart, ivend
          IF (soiltyp_subs(i) >= 3 .AND. zep_s(i) < 0.0_vpp) THEN
            ...
            ztrabpf  = ztraleav(i)*                   & ! plant covered part
                          (1.0_vpp - zfd_wi(i))*       & ! not water covered
                          (1.0_vpp - zf_snow(i))         ! not snow covered
            ...
            ELSE
              ...
              IF(zw_fr(i,kso)+ztrang(i,kso)*zdtdrhw/zdzhs(kso) &
                                .LT.zpwp(i,kso)) ztrang(i,kso) = 0.0_vpp
            ENDIF
            lhfl_pl(i,kso)= lh_v * ztrang(i,kso)
            ztrangs(i)    = ztrangs(i) + ztrang(i,kso)
          END IF  ! upwards directed potential evaporation only
  ...
  !$acc parallel async
  !$acc loop gang vector private(ze_sum, zzz, zxx)
  DO i = ivstart, ivend
    ze_sum = zdwsndt(i) + zdwidt(i) + zesoil(i) + ztrangs(i)
    IF (zep_s(i) < 0.0_vpp .AND. ze_sum < zep_s(i)) THEN
      zzz = zep_s(i)/ze_sum
    ...
    IF (itype_trvg == 3) THEN
      ...
      zzz = MAX(1.0_vpp, 0.1_vpp*(50.0_vpp - pabs(i)))
      plevap(i) = MAX(-6.0_vpp, MIN(0.0_vpp, plevap(i) + zzz*dt/lh_v *           &
                 (SUM(lhfl_pl(i,1:ke_soil_hy))+MAX(0.2_vpp,plcov(i))*75.0_vpp) ))
    ENDIF
    ...
    IF ( (tsnred(i) < 0.0_vpp) .AND. zep_s(i) < 0.0_vpp) THEN
      zzz = MAX(0.0_vpp,1.0_vpp-ABS(tsnred(i)))
      zxx = MIN(1.0_vpp,MAX(0.0_vpp,2.0_vpp-ABS(tsnred(i))))
...
!------------------------------------------------------------------------------
  ...
  ! Computation of derived constants
  z1d2dt = 1.0_vpp/zdt      ! 1./2*timestep
...
!    zclayf (i) = cclayf(mstyp)
  ...
  DO   kso = 1,ke_soil+1
    ...
    DO i = ivstart, ivend
      ...
      IF (kso<=ke_soil) THEN
        ztrangs(i) = ztrangs(i) + ztrang(i,kso)
        zdwgdt(i,kso) = 0.0_vpp
      END IF
      zflmg(i,kso)        = 0.0_vpp
      zrunoff_grav(i,kso) = 0.0_vpp
    END DO
  END DO      !soil layers
  !$acc end parallel
...
!     DO i = ivstart, ivend
!       zalpha_uf   = MAX (0.0_vpp, MIN(zmls(kso)/c_uf_h,1.0_vpp))
!       zroc(i,kso) = sa_uf(i) * (1.0_vpp - zalpha_uf) *  c_rhoc_bm  +       &
!                     (1.0_vpp - sa_uf(i) + sa_uf(i)*zalpha_uf) *  zroc(i,kso)
!     ENDDO
!   ENDDO
! END IF
...
!------------------------------------------------------------------------------
  ...
  IF (itype_interception == 1) THEN
    !$acc parallel async
    !$acc loop gang worker vector private(zrime, zfr_ice_free, zalf, zuv, zzz, zinf) &
    ...
    DO i = ivstart, ivend
      ...
      IF (zrs(i) >= 1.05_vpp*zrime .OR. zf_snow(i) >= 0.9_vpp) zrime = 0.0_vpp
      zrs(i) = zrs(i) - zrime
      ...
      zwinstr(i) = MAX(0.0_vpp,zwinstr(i))

      zwimax(i) = cwimax_ml*(1.0_vpp+ztfunc(i))*MAX(ztfunc(i), eps_soil, MAX(2.5_vpp*plcov(i),tai(i)))
      zalf   = SQRT(MAX(0.0_vpp,1.0_vpp - zwinstr(i)/zwimax(i)))

      ! water supply from interception store (if Ts above freezing)
      zuv    = SQRT ( u_10m(i)**2 + v_10m(i)**2 )
      zzz    = MAX(0.1_vpp, 0.4_vpp - 0.05_vpp*zuv)
      zinf   = MAX(0.0_vpp,zwinstr(i)-zzz*zwimax(i))*rho_w/ctau_i*        &
               (1.0_vpp+0.75_vpp*MAX(0.0_vpp,zuv-1.0_vpp))*(1.0_vpp-ztfunc(i))**2

      ! possible contribution of rain to infiltration
      !    IF (zrr(i)-eps_soil > 0.0_vpp) THEN
      !      zalf = MAX( zalf,                                                   &
      !             (zrhwddt*MAX(0.0_vpp, zwimax(i)-zwinstr(i)) + zinf)/zrr(i) )
      !      zalf = MAX( 0.01_vpp, MIN(1.0_vpp, zalf) )

      ! if rain falls onto snow, all rain is considered for infiltration
      ! as no liquid water store is considered in the snowpack
      IF (zwsnow(i) > 0.0_vpp) zalf = 0.0_vpp
      ...
      IF (lmulti_snow .AND. zwsnow(i) > 0.0_vpp) zalf = 1.0_vpp

      ! interception store; convective precip is taken into account with a 
      ! fractional area passed from the convection scheme
      zdwidtt  = zalf*(zrr(i)+(conv_frac(i)-1.0_vpp)*prr_con(i)) + zrime + zdwidt(i)-zinf
      zwinstr(i)  = zwin(i) + zdwidtt*zdtdrhw
      zwinstr(i)  = MAX(0.0_vpp, zwinstr(i)) !avoid negative values (security)
      zdwieps  = 0.0_vpp
      IF (zwinstr(i) > 0.0_vpp .AND. zwinstr(i) < 1.0E-4_vpp*eps_soil) THEN
      ...
      zro_wi       = zrhwddt*MAX( 0.0_vpp, zwinstr(i)-zwimax(i) )
      ...
      zvers(i) = zinf + (1.0_vpp - zalf)*zrr(i) + (1.0_vpp-conv_frac(i))*zalf*prr_con(i)

!     IF (lterra_urb .AND. (itype_eisa == 2)) THEN
!       !HW: this is just a reminder
!       !zvers * (1.0_vpp- isa(i,j) is the amount of water falling on non-impervious surface
...
!       ! unit w_imp: kg/m^2
!       w_imp(i) = MAX(w_imp(i) + zvers(i)*zdt + zeisa(i)*zdt , 0.0_vpp)

!       ! potential infiltration contribution from impervious to natural surface 
!       zisa_infil = MAX(w_imp(i) - c_isa_wmax, 0.0_vpp)/zdt
...
!       zro_inf  = MAX(0.0_vpp, zvers(i)*(1.0_vpp - fr_paved(i)) - zinfil(i) + &
!                               zisa_infil * fr_paved(i) * c_isa_runoff    )

!     ELSE
        ! Avoid infiltration for rock, ice and snow-covered surfaces
        zinfil(i) = zvers(i)*zrock(i)*(1.0_vpp - zf_snow(i))
...
!     ENDIF
      ...
      zwsnstr  = MAX(0.0_vpp, zwsnstr) ! avoid negative values (security)
      zdwseps  = 0.0_vpp
      IF (zwsnstr > 0.0_vpp .AND. zwsnstr < eps_soil) THEN
        ! shift marginal snow amounts to interception storage
!       IF (ztsnow_pm(i) > 0.0_vpp) THEN
...
!       END IF
  ...
  ELSEIF (itype_interception == 2) THEN
    ...
    DO i = ivstart, ivend
      ...
      zfr_ice_free     = 1.0_vpp-ziw_fr(i,1)/zporv(i,1)
      ...
      !      zf_wi  (j1,j2) = 1.0_vpp - exp(-0.5_vpp * plai(j1,j2)).  
      ...
      IF (ztsnow(i).gt.t0_melt) THEN
        ...
        zfcorr_wi=1.0_vpp ! Start value

        zdwidtt  = zf_wi(i)*zrr(i) + zf_wi(i)*zrs(i) + zewi(i)
        zwinstr(i)  = zwin (i) + zdtdrhw * zdwidtt
        zewi(i) = zewi(i) - MIN(0.0_vpp,zwinstr(i) * zrhwddt) ! Partial evaporation
      ...
      zro_wi      = MAX( 0.0_vpp, zwinstr(i)-zwimax(i) )   ! excess over zwimax -> runoff and infiltration
      zwinstr(i)  = MAX(0.0_vpp, zwinstr(i))                 ! avoid negative values (security)
      ...
      !   ((t(i,j,ke,nx)-270.15)/1.87E5_vpp + SQRT(u(i,j,ke,nx)*u(i,j,ke,nx)+v(i,j,ke,nx)*v(i,j,ke,nx))/1.56E5_vpp)
      ! Just for testing, using plcov
      zdwisndtt=plcov(i)*zrs(i) + plcov(i)*zesn(i) - zwisn(i)* &
                ((t(i)-270.15)/1.87E5_vpp + zuv/1.56E5_vpp)
      zwisnstr(i)  = zwisn (i) + zdtdrhw * zdwisndtt
      zwisnstr(i)  = MAX(0.0_vpp, zwisnstr(i)) !avoid negative values (security)

      ! Calculation of infiltration
      ! add rain and interception excess to water supply for infiltration
      zvers(i) =  zrhwddt*zro_wi + (1.0_vpp - zf_wi(i))*zrr(i) ! Test  Excess over zwimax will infiltrated

      ! maximum infiltration rate of the soil (rock/ice/water-exclusion
          !      zinfmx(i) = zrock(i)*zfr_ice_free *csvoro &
          !       *( cik1*MAX(0.5_vpp,plcov(i))*MAX(0.0_vpp,           &
      ...
      ! surface run-off (residual of potential minus actual infiltration)
      zro_inf     = MAX(0.0_vpp, zvers(i) - zinfil(i))

      ! Pond interception
      zdwpdtt  = zvers(i) - zinfil(i)  + zepd(i)
      zwpnstr(i)  = zwpn   (i) + zdtdrhw * zdwpdtt
      IF (zvers(i) - zinfil(i).ge.0.0_vpp) then
        zepd(i) = zepd(i) - MIN(0.0_vpp,zwpnstr(i) * zrhwddt) ! Partial evaporation
      ELSE
        zepd(i) = zepd(i) - MIN(0.0_vpp,zwpn(i) * zrhwddt) ! Partial evaporation
      END IF

      ! Final calculation
      zpercmax = 2.E-3_vpp
      zdwpdtt  = zvers(i) - zinfil(i)  + zepd(i)
      zwpnstr(i)  = MAX(0.0_vpp,zwpn (i) + zdtdrhw * zdwpdtt )
      zro_inf = MAX(0.0_vpp, (zwpnstr(i)  - zpercmax)*zrhwddt )   ! Pond overflow?
...
!CDIR NODEP,VOVERTAKE,VOB
...
#else
  !$acc parallel async
  !$acc loop gang worker vector private(zice_fr_kso, zice_fr_ksop1, zlw_fr_kso) &
  ...
  DO i = ivstart, ivend
    IF (soiltyp_subs(i)  >= 3) THEN
      ...
      zredp05  = 1.0_vpp-zfr_ice/MAX(zlw_fr_kso+zice_fr_kso,zlw_fr_ksop1+zice_fr_ksop1)
  
      ! interpolated scaled liquid water fraction at layer interface
      zlw_fr_ksop05  = 0.5_vpp*(zdzhs(2)*zlw_fr_kso+zdzhs(1)*zlw_fr_ksop1) / zdzms(2)
...
!DIR$ IVDEP, PREFERVECTOR
    DO ic = 1, icount_soil
      i=soil_list(ic)
#else
    !$acc loop gang worker vector private(zice_fr_ksom1, zice_fr_kso)      &
    ...
    DO i = ivstart, ivend
      IF (soiltyp_subs(i) >= 3) THEN
        ...
        ! above and below
        zlw_fr_ksom05 = 0.5_vpp*(zdzhs(kso-1)*zlw_fr_kso+   &
                               zdzhs(kso)*zlw_fr_ksom1)/zdzms(kso)
        zlw_fr_ksop05 = 0.5_vpp*(zdzhs(kso+1)*zlw_fr_kso+   &
                               zdzhs(kso)*zlw_fr_ksop1)/zdzms(kso+1)

        ! compute reduction factor for coefficients
        zfr_ice = MAX(zice_fr_kso,zice_fr_ksom1)
        zredm05 = 1.0_vpp-zfr_ice/MAX(zlw_fr_kso+zice_fr_kso,zlw_fr_ksom1+zice_fr_ksom1)
        zfr_ice = MAX(zice_fr_kso,zice_fr_ksop1)
        zredp05 = 1.0_vpp-zfr_ice/MAX(zlw_fr_kso+zice_fr_kso,zlw_fr_ksop1+zice_fr_ksop1)
        ...
        zagb (i,kso) = 1.0_vpp +zalfa*(zgam2m05+zgam2p05)*z1dgam1
        zagd (i,kso) = zlw_fr(i,kso)+                               &
                              z1dgam1*(-zklw_fr_ksop05+zklw_fr_ksom05)+ &
                              (1.0_vpp-zalfa)*z1dgam1*                &
  ...
  DO i = ivstart, ivend
    IF (soiltyp_subs(i) >= 3) THEN
      ...
      IF (soiltyp_subs(i) == 8 .AND. itype_mire == 1) THEN
        ...
        zlw_fr_ksom05 = 0.5_vpp*(zdzhs(ke_soil_hy_m-1)*zlw_fr_kso+ &
        ...
        zagb(i,ke_soil_hy_m) = 1.0_vpp+ zalfa*zgam2m05*z1dgam1
        zagc(i,ke_soil_hy_m) = 0.0_vpp
      ...
      ELSE
        ...
        zlw_fr_ksom05 = 0.5_vpp*(zdzhs(ke_soil_hy-1)*zlw_fr_kso+ &
        ...
        zagb(i,ke_soil_hy) = 1.0_vpp+ zalfa*zgam2m05*z1dgam1
        zagc(i,ke_soil_hy) = 0.0_vpp
...
!DIR$ IVDEP
    ...
    DO i = ivstart, ivend
      IF (soiltyp_subs(i) >= 3) THEN
#endif
        zzz = 1.0_vpp/(zagb(i,kso) - zaga(i,kso)*zagc(i,kso-1))
...
!DIR$ IVDEP
   DO ic = 1, icount_soil
    ...
    DO i = ivstart, ivend
      IF (soiltyp_subs(i) >= 3) THEN
        ...
        zlw_fr_ksom05 =0.5_vpp*(zdzhs(kso)*zlw_fr_ksom1+zdzhs(kso-1)*zlw_fr_kso) /zdzms(kso)
          !!$    zdlw_fr_ksom05= zredm05*zdw(i,kso)*EXP(zdw1(i,kso) *  &
          !!$                    (zporv(i,kso)-zlw_fr_ksom05)/(zporv(i,kso)-zadp(i,kso)) )
          !!$    zklw_fr_ksom05= zredm05*zkw(i,kso) * EXP(zkw1(i,kso)* &
          !!$                    (zporv(i,kso)-zlw_fr_ksom05)/(zporv(i,kso)-zadp(i,kso)) )
        ...
        IF (soiltyp_subs(i) == 8 .AND. itype_mire == 1) THEN
          IF (kso> ke_soil_hy_m) zdlw_fr_ksom05=0.0_vpp   ! no flux gradient
                                                         ! contribution below 2.5m
          IF (kso> ke_soil_hy_m) zklw_fr_ksom05=0.0_vpp   ! no gravitation flux below 2.5m
        ELSE

          IF (kso> ke_soil_hy) zdlw_fr_ksom05=0.0_vpp   ! no flux gradient
                                                       ! contribution below 2.5m
          IF (kso> ke_soil_hy) zklw_fr_ksom05=0.0_vpp   ! no gravitation flux below 2.5m
        END IF

        zflmg(i,kso) = (1.0_vpp-zalfa) * zflmg(i,kso) + & ! explicit flux component
                                  zalfa  * rho_w *    & ! implicit flux component
                     (zdlw_fr_ksom05 * (zlw_fr_kso_new+zice_fr_kso-zlw_fr_ksom1_new-zice_fr_ksom1) &
                       /zdzms(kso) - zklw_fr_ksom05)
        zredm = 1.0_vpp-zice_fr_kso/(zlw_fr_kso+zice_fr_kso)
        ...
        IF (w_so_new(i,kso).LT.1.01_vpp*zadp(i,kso)*zdzhs(kso)) THEN
          zklw_fr_kso_new=0.0_vpp
        ENDIF
        ...
        IF ((kso == ke_soil_hy+1).and.(itype_hydbound == 3)) THEN
          ...
          zrunoff_grav(i,ke_soil_hy)=zrunoff_grav(i,ke_soil_hy)+ zdhydcond_dlwfr / &
             (1.0_vpp-EXP(-zdhydcond_dlwfr/zdlw_fr_kso*0.5_vpp*zdzms(ke_soil_hy+1)))* zdelta_sm
        ENDIF
        IF ((kso == ke_soil_hy_m+1) .AND. (itype_mire == 1 .AND. soiltyp_subs(i) == 8)) THEN
          ...
          zrunoff_grav(i,ke_soil_hy_m)=zrunoff_grav(i,ke_soil_hy_m)+ zdhydcond_dlwfr / &
             (1.0_vpp-exp(-zdhydcond_dlwfr/zdlw_fr_kso*0.5_vpp*zdzms(ke_soil_hy_m+1)))* zdelta_sm
        END IF
#ifdef _OPENACC
      END IF
#endif
  ...
  IF (itype_mire == 1) THEN
    ...
    DO  kso = 1,ke_soil
      ...
      zro_sfak = zsf_heav(0.5_vpp + REAL(msr_off - kso,vpp))      ! 1.0 for 'surface runoff'
      zro_gfak = 1.0_vpp - zro_sfak                               ! 1.0 for 'ground runoff'
...
!DIR$ IVDEP
      ...
      DO i = ivstart, ivend
        ...
        IF (soiltyp_subs(i) >= 3) THEN
          ...
          IF (soiltyp_subs(i) == 8)  THEN
            zfmb_fak = MERGE(1.0_vpp, 0.0_vpp, kso==ke_soil_hy_m)
          ELSE
            zfmb_fak = MERGE(1.0_vpp, 0.0_vpp, kso==ke_soil_hy)
          END IF
          ...
          zro2   = zrhwddt*zdzhs(kso)*MAX(0.0_vpp, zwgn - zporv(i,kso))
          zkorr  = zrhwddt*zdzhs(kso)*MAX(0.0_vpp, zadp(i,kso) - zwgn )
...
!       ztalb = sa_uf(i) * ctalb_bm * alb_red_uf(i) + (1.0_vpp - sa_uf(i)) * Ctalb
!     ELSE
        ztalb = Ctalb
!     END IF

      zgstr =   sigma*(1.0_vpp - ztalb) * ( (1.0_vpp - zf_snow(i))* &
                zts(i) + zf_snow(i)*ztsnow_mult(i,1) )**4 + thbs(i)
      zthsnw(i) = - sigma*(1.0_vpp - ztalb)*ztsnow_mult(i,1)**4 + zgstr
      zthsoi(i) = - sigma*(1.0_vpp - ztalb)*zts(i)**4 + zgstr
      ...
      zlhfl_s(i) = (zts_pm(i)*lh_v + (1.0_vpp-zts_pm(i))*lh_s)*zverbo(i) &
                   / MAX(eps_div,(1.0_vpp - zf_snow(i)))  ! take out (1-f) scaling
      zqhfl_s(i) = zverbo(i)/ MAX(eps_div,(1.0_vpp - zf_snow(i)))  ! take out (1-f) scaling
    END DO
    !$acc end parallel


    !$acc parallel async
    !$acc loop gang worker vector private(zro, zrho_snowf)
    DO i = ivstart, ivend
      ! thawing of snow falling on soil with Ts > T0
      IF (ztsnow_pm(i)*zrs(i) > 0.0_vpp) THEN
      ...
      ELSEIF ((1.0_vpp-zts_pm(i))*zrr(i) > 0.0_vpp) THEN
        zsprs  (i) = lh_f*zrr(i)
      ...
      IF (zwsnew(i).GT.eps_soil) THEN
        ...
        IF(zdwsndt(i)-zrs(i)-zrr(i).GT.0.0_vpp) THEN

          wtot_snow_now(i,1) = MAX(wtot_snow_now(i,1) + zdwsndt(i)*zdtdrhw, 0.0_vpp)

          zhm_snow(i,1) = zhm_snow(i,1) - (zdwsndt(i)-zrs(i)-                             &
             zrr(i))*zdt/rho_i/2.0_vpp- zrs(i)*zdt/zrho_snowf/2.0_vpp- zrr(i)*zdt/rho_i/2.0_vpp
          zdzh_snow(i,1) = zdzh_snow(i,1) + (zdwsndt(i)-zrs(i)-zrr(i))*zdt/rho_i +        &
             zrs(i)*zdt/zrho_snowf + zrr(i)*zdt/rho_i

          rho_snow_mult_now(i,1) = MAX(wtot_snow_now(i,1)*rho_w/zdzh_snow(i,1), 0.0_vpp)
        ELSE

          wtot_snow_now(i,1) = MAX(wtot_snow_now(i,1) + (zrs(i)+zrr(i))*zdtdrhw, 0.0_vpp)

          zhm_snow(i,1)  = zhm_snow(i,1) - zrs(i)*zdt/zrho_snowf/2.0_vpp- &
                             zrr(i)*zdt/rho_i/2.0_vpp
          zdzh_snow(i,1) = zdzh_snow(i,1) + zrs(i)*zdt/zrho_snowf + zrr(i)*zdt/rho_i

          IF (wtot_snow_now(i,1) .GT. 0.0_vpp) THEN
            rho_snow_mult_now(i,1) = MAX(wtot_snow_now(i,1)*rho_w/zdzh_snow(i,1), 0.0_vpp)

            wtot_snow_now(i,1) = MAX(wtot_snow_now(i,1) &
                                  + (zdwsndt(i)-zrs(i)-zrr(i))*zdtdrhw,0.0_vpp)

            zhm_snow(i,1)  = zhm_snow(i,1) - (zdwsndt(i)-zrs(i)-zrr(i)) &
                                 *zdt/rho_snow_mult_now(i,1)/2.0_vpp
            zdzh_snow(i,1) = zdzh_snow(i,1) + (zdwsndt(i)-zrs(i)-zrr(i)) &
                                 *zdt/rho_snow_mult_now(i,1)
          ELSE
            rho_snow_mult_now(i,1) = 0.0_vpp
            zdzh_snow(i,1) = 0.0_vpp
          END IF

        END IF
      END IF
      h_snow_now(i) = 0.0_vpp
      sum_weight(i) = 0.0_vpp
    END DO
    !$acc end parallel
        ...
        IF (zwsnew(i) .GT. eps_soil) THEN
          IF (zwsnow(i) .GT. eps_soil) THEN
            IF (ksn == 1) THEN ! Limit top layer to max_toplaydepth
              zhh_snow(i,ksn) = -MAX( h_snow_now(i)-max_toplaydepth, h_snow_now(i)/REAL(ke_snow*(ke_snow-ksn),vpp) )
            ELSE IF (ksn == 2 .AND. ke_snow > 2) THEN ! Limit second layer to 8*max_toplaydepth
              zhh_snow(i,ksn) = MIN( 8.0_vpp*max_toplaydepth+zhh_snow(i,1), zhh_snow(i,1)/REAL((ke_snow-1)*(ke_snow-ksn),vpp) )
            ELSE ! distribute the remaining snow equally among the layers
              zhh_snow(i,ksn) = zhh_snow(i,k)/REAL(ke_snow-k*(ke_snow-ksn),vpp)
            ENDIF
          ELSE ! a newly generated snow cover will not exceed max_toplaydepth
            zhh_snow(i,ksn) = -h_snow_now(i)/REAL(ke_snow*(ke_snow-ksn),vpp)
          END IF
        END IF
      END DO
    END DO
    ...
    DO ksn = ke_snow,1,-1
      ...
      DO i = ivstart, ivend
        IF(zwsnew(i) .GT. eps_soil .AND. zwsnow(i) .GT. eps_soil) THEN
          ...
          z_old(i,ksn) = -sum_weight(i) - zdzh_snow(i,ksn)/2.0_vpp
    ...
    DO ksn = ke_snow,1,-1
      ...
      DO i = ivstart, ivend
        t_new  (i,ksn) = 0.0_vpp
        rho_new(i,ksn) = 0.0_vpp
        wl_new (i,ksn) = 0.0_vpp
      END DO

      !$acc loop gang vector collapse(2) private(weight)
      DO k = ke_snow,1,-1
        DO i = ivstart, ivend
          IF(zwsnew(i) .GT. eps_soil .AND. zwsnow(i) .GT. eps_soil) THEN

            weight = MIN(dz_old(i, k), &
                 &       z_old(i, k) + dz_old(i, k)*0.5_vpp &
                 &         - zhm_snow(i, ksn) + zdzh_snow(i, ksn)*0.5_vpp,&
                 &       zhm_snow(i, ksn) + zdzh_snow(i,ksn)*0.5_vpp &
                 &         - z_old(i, k) + dz_old(i, k)*0.5_vpp, &
                 &       zdzh_snow(i,ksn))

            weight = (weight + ABS(weight)) * 0.5_vpp
    ...
    ! MIN(z_old(i, k) + dz_old(i, k)/2.0_vpp, &
    ! &   zhm_snow(i, ksn) + zdzh_snow(i,ksn)/2.0_vpp) &
    ! - MAX(z_old(i, k) - dz_old(i, k)/2.0_vpp, &
    ! &     zhm_snow(i, ksn) - zdzh_snow(i, ksn)/2.0_vpp), &
      ...
      DO i = ivstart, ivend
        IF(zwsnew(i) .GT. eps_soil) THEN
          ...
          ELSE ! Remark: if there was now snow in the previous time step, snow depth 
            ...
            wtot_snow_now    (i,ksn) = zwsnew(i)/REAL(ke_snow,vpp)
          END IF
        END IF
      END DO
    END DO
    ...
    DO ksn = 1, ke_snow
      DO i = ivstart, ivend
        IF (zwsnew(i).GT.eps_soil) THEN
          zalas_mult(i,ksn) = 2.22_vpp*EXP(1.88_vpp*LOG(rho_snow_mult_now(i,ksn)/rho_i))
        END IF
      END DO
    END DO
    !$acc end parallel
    ...
    DO i = ivstart, ivend
      ...
      ! total forcing for uppermost soil layer
        !<em new solution
        !        zfor_s(i) = ( zrnet_s(i) + zshfl_s(i) + zlhfl_s(i) + zsprs(i) ) * (1.0_vpp - zf_snow(i)) &
        !                    + (1.0_vpp-ztsnow_pm(i)) * zgsb(i)
      zfor_s(i) = ( zrnet_s(i) + zshfl_s(i) + zlhfl_s(i) + zsprs(i) ) * (1.0_vpp - zf_snow(i))
        !em>

      IF(zwsnew(i) .GT. eps_soil) THEN
        IF(zextinct(i,1).gt.0.0_vpp) THEN
          zrnet_snow = zthsnw(i)
        ELSE
  ...
  ELSE  ! single-layer snow model
    ...
    !$acc parallel async
    !$acc loop gang worker vector private(ztalb, zgstr, zro, zalas)
    DO i = ivstart, ivend
      ! Estimate thermal surface fluxes:
      ! Estimate thermal surface fluxes over snow covered and snow free
      ! part of surface based on area mean values calculated in radiation
...
!     END IF
      ...
      ELSE IF (itype_canopy == 2) THEN
        ...
        zthsoi(i) = - sigma*(1.0_vpp - ztalb)*ztsk(i)**4 + zgstr
      END IF

      zthsnw(i) = - sigma*(1.0_vpp - ztalb)*ztsnow(i)**4 + zgstr
      ...
      ELSE IF (itype_canopy == 2) THEN
        ...
        zlhfl_s(i) = (ztsk_pm(i)*lh_v + (1.0_vpp-ztsk_pm(i))*lh_s)*zverbo(i) &
                     / MAX(eps_div,(1.0_vpp - zf_snow(i)))  ! take out (1-f) scaling
      END IF
      ...
      IF (ztsnow_pm(i)*zrs(i) > 0.0_vpp) THEN
        ...
        zzz = t0_melt-0.25_vpp
        zd  = (t_so_now(i,1)-zzz)*zroc(i,1)*zdzhs(1) + MAX(0.0_vpp,t_so_now(i,2)-zzz)*zroc(i,2)*zdzhs(2)
        zfr_melt = MIN(1.0_vpp,zd/(zdt*lh_f*zrs(i)))
        ...
        IF (zwinstr(i) > zwimax(i)) THEN  ! overflow of interception store
          ...
          zw_ovpv = MAX(0.0_vpp, (zw_fr(i,1)-zporv(i,1))*zdzhs(1)*zrhwddt + zdwgdt(i,1) )
          zdwgdt(i,1)= zdwgdt(i,1) - zw_ovpv
          runoff_s(i) = runoff_s(i) + zw_ovpv*zroffdt
        ENDIF                       ! overflow of interception store

      ! freezing of rain falling on soil with Ts < T0  (black-ice !!!)
      ELSEIF (zwsnow(i) == 0.0_vpp .AND.                            &
             (1.0_vpp-ztsnow_pm(i))*zrr(i) > 0.0_vpp) THEN
      ...
      IF (zwsnew(i).GT.eps_soil) THEN
        ...
        !
        zalas  = 2.22_vpp*EXP(1.88_vpp*LOG(zrho_snow(i)/rho_i))
        ...
        !        zalas   = 0.023_vpp+(2.290_vpp-0.023_vpp)* &
        !                               (7.750E-05_vpp*rho_snow(i,nx) + &
        !                                1.105E-06_vpp*prho_snow(i,nx)**2)
      ...
      IF (itype_canopy == 1) THEN
        ...
        zfor_s(i) = ( zrnet_s(i) + zshfl_s(i) + zlhfl_s(i) ) &
                       * (1.0_vpp - zf_snow(i)) + zsprs(i) &
                    + zf_snow(i) * (1.0_vpp-ztsnow_pm(i)) * zgsb(i)
      ...
      ELSE IF (itype_canopy == 2) THEN
        ...
        IF (cskinc < 0.0_vpp) THEN
          ztskn(i)  = ( MAX(5.0_vpp,skinc(i))*zts(i)                                     &
                      + zrnet_s(i)                                                      &
                      + cimpl*sigma*(1.0_vpp - ztalb)*ztsk(i)**4                          &
                      + zshfl_s(i) + zlhfl_s(i) + zsprs(i)                            ) &
                    / ( MAX(5.0_vpp,skinc(i)) + cimpl*sigma*(1.0_vpp - ztalb)*ztsk(i)**3 )
        ELSE
          ztskn(i)  = ( cskinc*zts(i)                                                   &
                      + zrnet_s(i)                                                      &
                      + cimpl*sigma*(1.0_vpp - ztalb)*ztsk(i)**4                          &
                      + zshfl_s(i) + zlhfl_s(i) + zsprs(i)                            ) &
                    / ( cskinc               + cimpl*sigma*(1.0_vpp - ztalb)*ztsk(i)**3 )
        END IF

        IF ((zwsnew(i) .GT. eps_soil) .OR. (zf_snow(i) .GT. 0.0_vpp)) ztskn(i) = ztsnow(i)

        ! total forcing for uppermost soil layer

        zfor_s(i) = ( zrnet_s(i)                                                        &
                    - cimpl*sigma*(1.0_vpp - ztalb)*ztsk(i)**3 *(ztskn(i)-ztsk(i))        &
                    + zshfl_s(i) + zlhfl_s(i)                                    )      &
                    * (1.0_vpp - zf_snow(i)) + zsprs(i)                                   &
                  +   zf_snow(i) * (1.0_vpp-ztsnow_pm(i)) * zgsb(i)
#endif
    ...
    IF (msg_level >= 20) THEN
      ...
      DO i = ivstart, ivend
        IF (soiltyp_subs(i) == 1) THEN  !1=glacier and Greenland
          IF ( ABS( zshfl_s(i) )  >  500.0_vpp  .OR. &
               ABS( zlhfl_s(i) )  > 2000.0_vpp ) THEN
...
!------------------------------------------------------------------------------
  ...
  ! if the multi-layer snow model is used, but zf_snow(i) < 1.0_vpp,
  ...
  !$acc parallel async
  !$acc loop gang vector collapse(2) private(zakb1, zakb2)
  DO kso = 2, ke_soil
    DO i = ivstart, ivend
      IF (.NOT. lmulti_snow .OR. sn_frac(i) < 1.0_vpp) THEN
        ...
        zagb(i,kso) = 1.0_vpp - zaga(i,kso) - zagc(i,kso)
        zagd(i,kso) = t_so_now(i,kso) +                                     &
               (1.0_vpp - zalfa)*( - zaga(i,kso)/zalfa*t_so_now(i,kso-1)+ &
  ...
  DO i = ivstart, ivend
!    IF (.NOT. lmulti_snow .OR. (zf_snow(i) < 1.0_vpp .AND. zwsnew(i).GT.eps_soil)) THEN
    IF (.NOT. lmulti_snow .OR. sn_frac(i) < 1.0_vpp) THEN
      ...
      zagb(i,1) = 1.0_vpp - zaga(i,1) - zagc(i,1)
      zagd(i,1) = t_so_now(i,1) + (1.0_vpp - zalfa)* (                 &
      ...
      zagd(i,0)    = zdzms(1) * zfor_s(i)/hzalam(i,1)+(1.0_vpp-zalfa)* &
                      (t_so_now(i,1) - t_s_now(i))
      zaga(i,ke_soil+1) = 0.0_vpp
      zagb(i,ke_soil+1) = 1.0_vpp
      zagc(i,ke_soil+1) = 0.0_vpp
  ...
  IF (lmulti_snow) THEN
    ...
    DO i = ivstart, ivend
      IF (sn_frac(i) > 0.0_vpp) THEN
        ...
        zagb(i,1) = 1.0_vpp - zaga(i,1) - zagc(i,1)
        zagd(i,1) = ztsnow_mult(i,1) + (1.0_vpp - zalfa)* (-zaga(i,1)/zalfa * t_s_now(i) + &
                    (zaga(i,1) + zagc(i,1))/zalfa * t_so_now(i,1) - zagc(i,1)/zalfa * t_so_now(i,2) )
        zaga(i,0) = 0.0_vpp
        zagb(i,0) = zalfa
        zagc(i,0) = -zalfa
        zagd(i,0) = zdzm_snow(i,1) * zfor_snow_mult(i)/zalas_mult(i,1)+(1.0_vpp-zalfa)* &
                   (t_so_now(i,1) - t_s_now(i))
        zagc(i,0) = zagc(i,0)/zagb(i,0)
        zagd(i,0) = zagd(i,0)/zagb(i,0)

        ! Lowermost soil layer, Dirichlet boundary condition
        zaga(i,ke_snow + ke_soil+1) = 0.0_vpp
        zagb(i,ke_snow + ke_soil+1) = 1.0_vpp
        zagc(i,ke_snow + ke_soil+1) = 0.0_vpp
        ...
        zagb(i,ke_snow) = 1.0_vpp - zaga(i,ke_snow) - zagc(i,ke_snow)
        zagd(i,ke_snow) = ztsnow_mult(i,ke_snow) + &
          &     (1.0_vpp - zalfa)*( - zaga(i,ke_snow)/zalfa*ztsnow_mult(i,ke_snow-1) + &
        ...
        zagb(i,ke_snow+1) = 1.0_vpp - zaga(i,ke_snow+1) - zagc(i,ke_snow+1)
        zagd(i,ke_snow+1) = t_so_now(i,1) + &
          &    (1.0_vpp - zalfa)*( - zaga(i,ke_snow+1)/zalfa*ztsnow_mult(i,ke_snow) + &
    ...
    ! Snow layers
    !$acc parallel async
    !$acc loop gang vector collapse(2) private(zakb)
    DO ksn = 2, ke_snow-1
      DO i = ivstart, ivend
        IF (sn_frac(i) > 0.0_vpp) THEN
          ...
          zagb(i,ksn) = 1.0_vpp - zaga(i,ksn) - zagc(i,ksn)
          zagd(i,ksn) = ztsnow_mult(i,ksn) + &
            &     (1.0_vpp - zalfa)*( - zaga(i,ksn)/zalfa*ztsnow_mult(i,ksn-1) + &
    ...
    ! Soil layers
    !$acc parallel async
    !$acc loop gang vector collapse(2) private(kso, zakb1, zakb2)
      ...
      DO i = ivstart, ivend
        IF (sn_frac(i) > 0.0_vpp) THEN
          ...
          zagd(i,ksn) = t_so_now(i,kso) + &
            &    (1.0_vpp - zalfa)*( - zaga(i,ksn)/zalfa*t_so_now(i,kso-1) +  &
...
!in case of thin snowpack (less than zswitch), apply single-layer snow model
    ...
    DO i = ivstart, ivend
      IF (sn_frac(i) > 0.0_vpp) THEN
        ...
        zswitch(i) = MAX(zswitch(i),1.E-03_vpp)
      ELSE
        zswitch(i) = 0.0_vpp
      END IF
    END DO
    ...
    DO i = ivstart, ivend
      IF(zwsnew(i) .LT. zswitch(i) .AND. sn_frac(i) > 0.0_vpp) THEN
        ztsn  (i) = t_so_now(i,1)
        tmp_num(i) = ztsnow_mult(i,1) + zdt*2.0_vpp*(zfor_snow_mult(i) - zgsb(i))  &
                           /zrocs(i)/(zswitch(i)/rho_snow_mult_now(i,1)*rho_w) &
                           &- ( ztsn(i) - zts(i) )
        zalas  = 2.22_vpp*EXP(1.88_vpp*LOG(rho_snow_mult_now(i,1)/rho_i))

        ztsnow_im    = - zrhoch(i) * (cp_d + zdqvtsnow(i) * lh_s) - zalas/h_snow_now(i)
        zfak  = MAX(eps_soil,1.0_vpp - zdt*zalfa*ztsnow_im/zrocs(i)/h_snow_now(i))
...
!    END DO
    ...
    DO ksn = 1, ke_snow
      DO i = ivstart, ivend
        IF(zwsnew(i) .GT. eps_soil .and. zwsnew(i) .LT. zswitch(i)) THEN
          ...
          ELSE IF(zfor_snow_mult(i)-zgsb(i) .GT. 0.0_vpp) THEN
            ztsnown_mult(i,ksn) = ztsnow_mult(i,ksn) + &
              (zfor_snow_mult(i)-zgsb(i))*zdt/(chc_i*wtot_snow_now(i,ksn))/rho_w/REAL(ke_snow,vpp)
          END IF
        END IF
      END DO
    END DO
  ...
  IF(lmulti_snow) THEN
    ...
    DO kso = 1, ke_soil+1
      DO i = ivstart, ivend
        t_so_new(i,kso) = t_so_snow_new(i,kso)*sn_frac(i) + t_so_free_new(i,kso)*(1.0_vpp - sn_frac(i))
      END DO
    END DO
    !$acc end parallel
  ELSE
...
!DIR$ IVDEP
      DO ic=1,icount_soil
        i=soil_list(ic)
#else
      !$acc loop gang worker vector private(ztx, zliquid, znen, zfak, zdelwice) &
...
!US         zw_m(i) = zw_m(i)*EXP(-zedb(i)*LOG((t_so_new(i,kso) - t0_melt)/(t_so_new(i,kso)*zxx)) )
            ...
            znen   = 1.0_vpp-zaa(i)*EXP(zb_por(i)*LOG(zporv(i,kso)*zdzhs(kso)/zliquid))
            ztx    = t0_melt/znen
          ENDIF
          ztx      = MIN(t0_melt,ztx)
          ...
          IF (t_so_new(i,kso) > t0_melt .AND. w_so_ice_now(i,kso) > 0.0_vpp) THEN
            ! melting point adjustment (time scale 30 min)
            zdelwice = - MIN(w_so_ice_now(i,kso), zdwi_scal*(t_so_new(i,kso)-t0_melt)*zfak)
          ELSE IF (zdelwice < 0.0_vpp) THEN ! this branch contains cases of melting and freezing
            ...
            zdelwice = - MIN( - zdelwice,MAX(2.5_vpp*zdwi_scal,0.5_vpp*(t_so_new(i,kso)-t_so_now(i,kso)))*zfak)
          ELSE
            zdelwice = MIN(zdelwice,MAX(zargu,0.0_vpp))
          ENDIF
          IF (zdelwice > 0.0_vpp) THEN
            ! limit latent heat release due to freezing to half the differene from the melting point
            zdelwice = MIN(zdelwice,0.5_vpp*MAX(0.0_vpp,(t0_melt-t_so_new(i,kso)))*zfak)
          ENDIF
...
!------------------------------------------------------------------------------
  !$acc parallel async
  !$acc loop gang worker vector private(zrnet_snow, zfor_snow, zalas, ztsnow_im, zfak)
  DO i = ivstart, ivend
    ...
    IF(.NOT. lmulti_snow) THEN
      ...
      IF (ztsnow(i) < t0_melt .AND. zwsnew(i) > eps_soil) THEN
        ztsnown(i) = ztsnow(i) + zdt*2.0_vpp*(zfor_snow - zgsb(i))  &
        ...
        ztsnow_im    = - zrhoch(i) * (cp_d + zdqvtsnow(i) * lh_s)       &
                                         - zalas/zdz_snow_fl(i)
        zfak  = MAX(eps_div,1.0_vpp - zdt*zalfa*ztsnow_im/zrocs(i))
  ...
  IF (msg_level >= 20) THEN
    ...
    DO i = ivstart, ivend
!     IF (soiltyp_subs(i) == 1) THEN  !1=glacier and Greenland
      IF ( ABS( zshfl_s(i)    * (1.0_vpp-zf_snow(i)) )  >  700.0_vpp .OR. &
           ABS( zlhfl_s(i)    * (1.0_vpp-zf_snow(i)) )  > 2000.0_vpp        ) THEN
        write(*,*) 'soil soil: ', zshfl_s(i), zrhoch(i),zth_low(i),t(i),zts(i), &
          '  ...LHF...  ',        zlhfl_s(i), zts_pm(i),zverbo(i),zf_snow(i),qv(i),qv_s(i), &
          '  ...CH,CM...  ', tch(i), tcm(i)
      ENDIF
      IF ( ABS( zshfl_snow(i) )  >  500.0_vpp  .OR. &
           ABS( zlhfl_snow(i) )  > 2000.0_vpp ) THEN
...
! melting snow. In cases this melting may be postponed to the next time step.
  ...
  IF (.NOT. lmulti_snow) THEN
    ...
    !$acc parallel async
    !$acc loop gang worker vector private(ztsnownew, ze_avail, ze_total, zfr_melt) &
    ...
    DO i = ivstart, ivend
      ...
      IF (zwsnew(i) > eps_soil) THEN        ! points with snow cover only
        ...
        IF (ztsnown(i) > t0_melt .AND. t_so_new(i,1) < t0_melt ) THEN
          ...
          zdwsnm(i)    = MIN(1.5_vpp*rho_snow_now(i)/rho_w,zwsnew(i))* &
                .5_vpp*(ztsnown(i) - (t0_melt - eps_temp))/ &
               (.5_vpp* (zts(i) - (t0_melt - eps_temp)) - lh_f/chc_i)
          ...
          zredfu       = MAX( 0.0_vpp,  MIN( 1.0_vpp, (zw_fr(i,1) -  &
                         zfcap(i,1))/MAX(zporv(i,1)-zfcap(i,1), eps_div)))
          zdwgdt(i,1)  = zdwgdt(i,1) + zdwgme*(1.0_vpp - zredfu)
          zro          = meltrate(i) - zdwgme*(1.0_vpp - zredfu)
        ...
        IF (t_so_new(i,1) >= t0_melt) THEN
          ...
          IF (ztsnownew > t0_melt) THEN
            ...
            zro           = (1.0_vpp - zrock(i))*zdwsnm(i)      ! surface runoff
            zredfu        = MAX( 0.0_vpp,  MIN( 1.0_vpp, (zw_fr(i,1) -  &
                            zfcap(i,1))/MAX(zporv(i,1)-zfcap(i,1), eps_div)))
            zdwgdt(i,1) = zdwgdt(i,1) + zdwgme*(1.0_vpp - zredfu)
            ...
            ! zro-, zdw_so_dt-correction in case of pore volume overshooting
            zw_ovpv = MAX(0.0_vpp, zw_fr(i,1)* zdzhs(1) * zrhwddt +  &
                       zdwgdt(i,1) - zporv(i,1) * zdzhs(1) * zrhwddt)
            zro = zro + zw_ovpv
            zdwgdt(i,1)= zdwgdt(i,1) - zw_ovpv


            IF (zfr_melt > 0.9999_vpp) zdwsndt(i)= -zwsnow(i)*zrhwddt
            runoff_s (i)= runoff_s(i) + zro*zroffdt
  ...
  ELSE       ! multi-layer snow scheme
    ...
    DO i = ivstart, ivend
      ...
      ze_rad(i) = 0.0_vpp
      IF(zextinct(i,1).gt.0.0_vpp) ze_rad(i) = zf_snow(i) * sobs(i)
    ...
    DO ksn = 1,ke_snow
      ...
      DO i = ivstart, ivend
        IF (zwsnew(i) > eps_soil) THEN        ! points with snow cover only
          ...
          IF(zextinct(i,ksn).eq.0.0_vpp) THEN
            ze_in = ze_out(i)
          ELSE
            ...
            ELSE
              ...
              ze_in = ze_out(i) + ze_rad(i) * (1.0_vpp - zcounter(i))
          ...
          IF(wtot_snow_now(i,ksn) .LE. wliq_snow_now(i,ksn)) THEN
            ...
            wliq_snow_now(i,ksn) = 0.0_vpp
            wtot_snow_now(i,ksn) = 0.0_vpp
            zdzh_snow(i,ksn)    = 0.0_vpp
            rho_snow_mult_now(i,ksn)  = 0.0_vpp
          ELSE
            IF(zrefr(i) .GT. 0.0_vpp .OR. zmelt(i) .GT. 0.0_vpp) THEN
              zadd_dz = 0.0_vpp
              zadd_dz = MAX(zrefr(i),0.0_vpp)*(-1.0_vpp + 1.0_vpp/rho_i*rho_w)*zdt
              zadd_dz = MAX(zmelt(i),0.0_vpp)*(-1.0_vpp/zrho_dry_old(i)*rho_w &
                &       + 1.0_vpp)*zdt
              zdzh_snow(i,ksn)   = zdzh_snow(i,ksn) + zadd_dz
              rho_snow_mult_now(i,ksn) = MAX(wtot_snow_now(i,ksn)*rho_w &
                &                            /zdzh_snow(i,ksn),0.0_vpp)
              IF(wtot_snow_now(i,ksn) .LE. 0.0_vpp) zdzh_snow(i,ksn) = 0.0_vpp
            ...
            zsn_porosity = 1.0_vpp - (rho_snow_mult_now(i,ksn)/rho_w -  &
                           wliq_snow_now(i,ksn)/zdzh_snow(i,ksn))/rho_i*rho_w - &
                           wliq_snow_now(i,ksn)/zdzh_snow(i,ksn)
            zsn_porosity = MAX(zsn_porosity,cwhc + 0.1_vpp)
            ...
            IF (wliq_snow_now(i,ksn)/zdzh_snow(i,ksn) .GT. cwhc) THEN
              ...
              IF(zdzh_snow(i,ksn) .LT. eps_soil*0.01_vpp) THEN
                wliq_snow_now(i,ksn) = 0.0_vpp
                wtot_snow_now(i,ksn) = 0.0_vpp
                zdzh_snow(i,ksn)     = 0.0_vpp
                rho_snow_mult_now(i,ksn)  = 0.0_vpp
    ...
    !$acc parallel async
    !$acc loop gang vector collapse(2) private(zdens_old, zeta)
    DO ksn = 2, ke_snow
      DO i = ivstart, ivend
        IF (zwsnew(i) > eps_soil) THEN        ! points with snow cover only
          IF(rho_snow_mult_now(i,ksn) .LT. 600.0_vpp .AND. &
            rho_snow_mult_now(i,ksn) .NE. 0.0_vpp) THEN
            zdens_old = rho_snow_mult_now(i,ksn)
            zeta =         &! compactive viscosity of snow
              ca2*EXP(19.3_vpp*rho_snow_mult_now(i,ksn)/rho_i)* &
              EXP(67300.0_vpp/8.31_vpp/ztsnownew_mult(i,ksn))
...
#endif
      zfor_s(i)=0.0_vpp ! no soil forcing is needed at this step
    ...
    DO i = ivstart, ivend
      IF (w_snow_now(i) > eps_soil .AND. w_snow_new(i) < eps_soil) THEN ! Snow vanished during time step
        ...
        zagd(i,kso) = t_so_new(i,kso) +                                     &    ! distribute heat in (*)
               (1.0_vpp - zalfa)*( - zaga(i,kso)/zalfa*t_so_new(i,kso-1)+ &
...
!CDIR NODEP,VOVERTAKE,VOB
  ...
  DO i = ivstart, ivend
    IF (w_snow_now(i) > eps_soil .AND. w_snow_new(i) < eps_soil) THEN ! Snow vanished during time step
      ...
      zagc(i,  1) = -zalfa*zdt*zakb2/(zdzhs(1)*zdzms(2))
      zagb(i,  1) = 1.0_vpp - zaga(i,1) - zagc(i,1)
      zagd(i,  1) = t_so_new(i,1) + (1.0_vpp - zalfa)* (                 &
...
#else
    !$acc loop gang worker vector                     &
    !$acc private(ztx, zliquid, znen, zfak, zdelwice) &
    !$acc private(zwso_new, zargu)
    DO i = ivstart, ivend
      IF (w_snow_now(i) > eps_soil .AND. w_snow_new(i) < eps_soil) THEN ! Snow vanished during time step
#endif
...
!US         zw_m(i) = zw_m(i)*EXP(-zedb(i)*LOG((t_so_new(i,kso) - t0_melt)/(t_so_new(i,kso)*zxx)) )
          ...
          ELSE
            zdelwice = MIN(zdelwice,MAX(zargu,0.0_vpp))
            ! limit latent heat release due to freezing to half the differene from the melting point
            zdelwice = MIN(zdelwice,0.5_vpp*(t0_melt-t_so_new(i,kso))*zfak)
          ENDIF
          w_so_ice_new(i,kso) = w_so_ice_now(i,kso) + zdelwice
          t_so_new(i,kso) = t_so_new(i,kso) + zdelwice/zfak
  ...
  DO i = ivstart, ivend
    IF (w_snow_new(i) <= eps_soil) THEN
      ...
      w_snow_new(i) = 0.0_vpp
      t_snow_new(i) = t_so_new(i,0)
    ENDIF

    IF (w_i_new(i) <= 1.0E-4_vpp*eps_soil) w_i_new(i) = 0.0_vpp
  END DO
  !$acc end parallel
!<JH
    ...
    DO ksn = 1, ke_snow
      DO i = ivstart, ivend
        IF (w_snow_new(i) <= eps_soil) THEN
          ...
          wliq_snow_new(i,ksn) = 0.0_vpp
          wtot_snow_new(i,ksn) = 0.0_vpp
          rho_snow_mult_new(i,ksn) = 0.0_vpp
          dzh_snow_new(i,ksn) = 0.0_vpp
        ENDIF
      END DO
    END DO
    !$acc end parallel
  ...
  IF(.NOT. lmulti_snow) THEN
    ...
    !$acc parallel async
    !$acc loop gang worker vector private(zzz, ztau_snow, zrhosmax, zrho_snowe) &
    ...
    DO i = ivstart, ivend
      ...
      ztau_snow = MAX(0.05_vpp,MIN(crhosmaxt,ztau_snow)) ! use 20 days in combination with
                                                        ! temperature-dependent equilibrium density
      zrhosmax  = crhosmax_tmin+MAX(-0.25_vpp,zzz)*(crhosmax_ml-crhosmax_tmin)
      zrho_snowe= MAX(rho_snow_now(i),zrhosmax+(rho_snow_now(i)-zrhosmax)* &
                  EXP(-ztau_snow*zdt/86400.0_vpp) )
      ...
      IF (zzz > 0.5_vpp*eps_soil .AND. zth_low(i) > t0_melt + 0.5_vpp) THEN
        ! part of the new snow that accumulates on the ground
        zxx = MAX(0.0_vpp, zzz*(t0_melt + 1.0_vpp - zth_low(i))*2.0_vpp)
        !
        ...
        zro           = (1.0_vpp - zrock(i))*zinf      ! surface runoff
        zredfu        = MAX( 0.0_vpp,  MIN( 1.0_vpp, (zw_fr(i,1) -  &
                        zfcap(i,1))/MAX(zporv(i,1)-zfcap(i,1), eps_div)))
        zdwgdt(i,1) = zdwgdt(i,1) + zdwgme*(1.0_vpp - zredfu)
        zro           = zro + zdwgme*zredfu    ! Infiltration not possible
      ...
      IF (soiltyp_subs(i) /= 1) THEN
        rho_snow_new(i)  = (w_snow_now(i)+zzz) / &
         ( MAX(w_snow_now(i),eps_soil)/zrho_snowe + zzz/( (1.0_vpp-zgrfrac)*zrho_snowf + zgrfrac*zrho_grauf) )
  ...
  ELSE   ! new snow scheme
    ...
    DO ksn = 1,ke_snow
      DO i = ivstart, ivend
        IF (w_snow_new(i) .GT. eps_soil) THEN
          IF (ksn == 1) THEN ! Limit top layer to max_toplaydepth
            zhh_snow(i,ksn) = -MAX( h_snow_new(i)-max_toplaydepth, h_snow_new(i)/REAL(ke_snow*(ke_snow-ksn),vpp) )
          ELSE IF (ksn == 2 .AND. ke_snow > 2) THEN ! Limit second layer to 8*max_toplaydepth
            zhh_snow(i,ksn) = MIN( 8.0_vpp*max_toplaydepth+zhh_snow(i,1), zhh_snow(i,1)/REAL((ke_snow-1)*(ke_snow-ksn),vpp) )
          ELSE ! distribute the remaining snow equally among the layers
            zhh_snow(i,ksn) = zhh_snow(i,k)/REAL((ke_snow-k)*(ke_snow-ksn),vpp)
    ...
    DO ksn = ke_snow,1,-1
      ...
      DO i = ivstart, ivend
        IF (w_snow_new(i) .GT. eps_soil) THEN
          ...
          z_old(i,ksn) = -sum_weight(i) - dzh_snow_new(i,ksn)/2.0_vpp
    ...
    DO i = ivstart, ivend
       IF(w_snow_new(i) .GT. eps_soil) THEN
         zhm_snow(i,1) = (-h_snow_new(i) + zhh_snow(i,1))/2.0_vpp
      ...
      DO k = ke_snow,1,-1
        DO i = ivstart, ivend
          IF(w_snow_new(i) .GT. eps_soil) THEN
            ...
            weight = MIN(&
                 dz_old(i,k), &
                 z_old(i,k) + dz_old(i,k)/2.0_vpp &
                 - zhm_snow(i,ksn) + dzh_snow_new(i,ksn)/2.0_vpp , &
                 zhm_snow(i,ksn) + dzh_snow_new(i,ksn)/2.0_vpp &
                 - z_old(i,k) + dz_old(i,k)/2.0_vpp, &
                 dzh_snow_new(i,ksn))

            weight = (weight + ABS(weight)) * 0.5_vpp / dzh_snow_new(i,ksn)
    ...
    DO i = ivstart, ivend
      ...
      ELSE !JH
        rho_snow_new(i) = 250.0_vpp ! workaround need to be inspected!!
      END IF
      IF(w_snow_new(i) > eps_soil) THEN
        ! linear extrapolation from t_snow_mult_new(i,2) and t_snow_mult_new(i,1) to t_snow_mult_new(i,0)
        t_snow_mult_new(i,0) = (t_snow_mult_new(i,1)*(2.0_vpp*dzh_snow_new(i,1)+dzh_snow_new(i,2))- &
        ...
        t_snow_mult_new(i,0) = MIN(273.15_vpp,t_snow_mult_new(i,0),t_snow_mult_new(i,1)+5.0_vpp)
        t_snow_mult_new(i,0) = MAX(t_snow_mult_new(i,0),t_snow_mult_new(i,1)-5.0_vpp)
      END IF
      t_snow_new(i) = t_snow_mult_new (i,0)
    END DO
    !$acc end parallel
 ...
 ENDIF ! lmulti_snow
  ...
  IF(PRESENT(zshfl_sfc)) THEN
    ...
    !$acc loop gang vector
    DO i = ivstart, ivend
      zshfl_sfc(i) = zshfl_s(i)*(1.0_vpp - zf_snow(i)) + zshfl_snow(i)*zf_snow(i)
      zlhfl_sfc(i) = zlhfl_s(i)*(1.0_vpp - zf_snow(i)) + zlhfl_snow(i)*zf_snow(i)
      zqhfl_sfc(i) = zqhfl_s(i)*(1.0_vpp - zf_snow(i)) + zqhfl_snow(i)*zf_snow(i)
...
!     IF (ABS(t_s_now(i)-t_s_new(i)) > 25.0_vpp) THEN
      IF (i== mvid .AND. iblock == mbid .AND. my_cart_id == mcid) THEN
#ifdef _OPENMP
       IF (my_thrd_id == mtid) THEN
#endif
...
REAL (KIND=vpp) FUNCTION zsf_heav (zstx)
  ! Heaviside function

  !$acc routine seq
  REAL (KIND=vpp), INTENT(IN)  :: zstx
  zsf_heav = 0.5_vpp + SIGN (0.5_vpp, zstx)

END FUNCTION zsf_heav

!==============================================================================

REAL (KIND=vpp) FUNCTION zsf_psat_iw  (zstx, z2iw, z4iw)
  ! Saturation water vapour pressure over ice or water depending on temperature "zstx"

  !$acc routine seq
  REAL (KIND=vpp), INTENT(IN)  :: zstx, z2iw, z4iw
  zsf_psat_iw = REAL(b1_wp,vpp)*EXP(z2iw*(zstx - REAL(b3_wp,vpp))/(zstx - z4iw))

END FUNCTION zsf_psat_iw

!==============================================================================

REAL (KIND=vpp) FUNCTION zsf_qsat  (zspsatx, zspx)
  ...
  !$acc routine seq
  REAL (KIND=vpp), INTENT(IN)  :: zspsatx, zspx
  zsf_qsat = REAL(rdv_wp,vpp)*zspsatx/(zspx - REAL(o_m_rdv_wp,vpp)*zspsatx)

END FUNCTION zsf_qsat

!==============================================================================

REAL (KIND=vpp) FUNCTION zsf_dqvdt_iw (zstx, zsqsatx, z4iw, z234iw)
  ...
  !$acc routine seq
  REAL (KIND=vpp), INTENT(IN)  :: zstx, zsqsatx, z4iw, z234iw
  zsf_dqvdt_iw = z234iw*(1.0_vpp + REAL(rvd_m_o_wp,vpp)*zsqsatx)*zsqsatx/(zstx - z4iw)**2
...
REAL (KIND=vpp) FUNCTION watrcon_RT(ks,lw,kw1,pv,adp)

  !$acc routine seq
  REAL (KIND=vpp), INTENT(IN)  :: ks,lw,kw1,pv,adp
  watrcon_RT=ks*EXP(kw1*MAX(0.0_vpp,pv-lw)/(pv-adp))

END FUNCTION watrcon_RT

!==============================================================================

REAL (KIND=vpp) FUNCTION watrdiff_RT(ds,lw,dw1,pv,adp)

  !$acc routine seq
  REAL (KIND=vpp), INTENT(IN)  ::  ds,lw,dw1,pv,adp
  watrdiff_RT = ds*EXP(dw1*MAX(0.0_vpp,pv-lw)/(pv-adp))
