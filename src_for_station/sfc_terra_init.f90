...
! External parameters
  ...
  REAL    (KIND=vpp) ::  & ! Soil and plant parameters
!
...
! Prepare basic surface properties (for land-points only)
  ...
  DO i = ivstart, ivend
    ...
    zbwt  (i)   = MAX(0.001_vpp,rootdp(i))   ! Artificial minimum value
                                            ! for root depth
    ...
    zpsis (i)   = -zpsi0 * 10.0_vpp**(1.88_vpp-0.013_vpp*zsandf(i))
  ...
  wso_ice_tolerance  = 1.05_vpp! 5% deviation allowed
  rwso_ice_tolerance = 1.0_vpp/wso_ice_tolerance
...
! moisture to zero in case of ice and rock
  ...
  DO kso   = 1,ke_soil+1
    ...
    DO i = ivstart, ivend
      IF (m_styp(i) >= 3) THEN
        ! soil list
        w_so_now (i,kso) = MAX(w_so_now(i,kso), 1.01_vpp*zadp(i,kso)*zdzhs(kso) )
        w_so_new (i,kso) = MAX(w_so_new(i,kso), 1.01_vpp*zadp(i,kso)*zdzhs(kso) )

#ifdef __COSMO__
        !UB: clipping of soil water content to its maximum allowed value:
        w_so_now (i,kso) = MIN(w_so_now(i,kso), 1.0_vpp*zporv(i,kso)*zdzhs(kso))
        w_so_new (i,kso) = MIN(w_so_new(i,kso), 1.0_vpp*zporv(i,kso)*zdzhs(kso))
#endif
      ELSE
        ! rock and ice list
        w_so_now (i,kso) = 0.0_vpp
        w_so_new (i,kso) = 0.0_vpp
      ENDIF
    END DO
  END DO
  !$acc end parallel
...
!   --------------------------------------------
    ...
    IF (.NOT. lana_rho_snow) THEN
       ...
       IF(lmulti_snow) THEN
         ...
         !$acc loop gang vector
         DO i = ivstart, ivend
           rho_snow_mult_now(i,ksn) = 250.0_vpp    ! average initial density
           t_snow_mult_now  (i,ksn) = t_snow_now(i)
           wliq_snow_now    (i,ksn) = 0.0_vpp
           ! Limit top layer to max_toplaydepth
           dzh_snow_now     (i,ksn) = MIN(max_toplaydepth, w_snow_now(i)/REAL(ke_snow,vpp) &
         ...
         DO ksn = 2, ke_snow
           ...
           DO i = ivstart, ivend
             ...
             IF (k == ksn) THEN
               dzh_snow_now   (i,ksn) = MIN(8.0_vpp*max_toplaydepth, (w_snow_now(i)-wtot_snow_now(i,1)) &
                    &                            /REAL(ke_snow-1,vpp)*rho_w/rho_snow_mult_now(i,ksn) )
             ELSE
               dzh_snow_now   (i,ksn) = (w_snow_now(i)-wtot_snow_now(i,k))/REAL(ke_snow-k,vpp) &
                    &                            *rho_w/rho_snow_mult_now(i,ksn)
             ENDIF
             wtot_snow_now    (i,ksn) = dzh_snow_now(i,ksn)*rho_snow_mult_now(i,ksn)/rho_w
           ENDDO
       ...
       ELSE
         ...
         !$acc loop gang vector
         DO i = ivstart, ivend
           rho_snow_now(i) = 250.0_vpp    ! average initial density
           IF (l2lay_rho_snow) THEN
             !US is second dim really  2??
             rho_snow_mult_now(i,1) = 250.0_vpp
             rho_snow_mult_now(i,2) = 250.0_vpp
           ENDIF
         ENDDO
         !$acc end parallel
       ENDIF
...
!US so far for COSMO, the rest is ICON
    ...
    ELSE
      ...
      IF (lmulti_snow) THEN
        ...
        !$acc loop gang vector
        DO i = ivstart, ivend
          zw_snow_old  (i) = 0.0_vpp
          zrho_snow_old(i) = 0.0_vpp
          zicount1(i)      = 0
          zicount2(i)      = 0
        ...
        !$acc loop gang vector
        DO i = ivstart, ivend
          zrho_snow_old(i) = zw_snow_old(i) / MAX(zrho_snow_old(i),1.0E-09_vpp) *rho_w
        END DO
        !$acc end parallel
        ...
        DO ksn = 1, ke_snow
          ...
          DO i = ivstart, ivend
            ...
            ELSE
              IF(rho_snow_now(i) .EQ. 0.0_vpp) rho_snow_now(i) = 250.0_vpp
              IF (ksn == 1) THEN ! Limit top layer to max_toplaydepth
                IF (t_snow_now(i) > 268.15_vpp) THEN
                  rho_snow_mult_now(i,ksn) = MIN(400.0_vpp,rho_snow_now(i))
                ELSE ! at very low temperatures, the near-surface snow is usually not subject to strong densification
                  rho_snow_mult_now(i,ksn) = MIN(rho_snow_now(i),400.0_vpp+6.0_vpp* &
                    (MAX(228.15_vpp,t_snow_now(i))-268.15_vpp)                      )
                ENDIF
                dzh_snow_now(i,ksn) = MIN(max_toplaydepth, w_snow_now(i)/REAL(ke_snow,vpp) &
                  &                  *rho_w/rho_snow_mult_now(i,ksn))
              ELSE ! distribute the remaining snow equally among the layers
                ...
                IF (ksn == k) THEN
                  dzh_snow_now(i,ksn) = MIN(8.0_vpp*max_toplaydepth, (w_snow_now(i)-wtot_snow_now(i,1)) &
                      &                 /REAL(ke_snow-1,vpp)*rho_w/rho_snow_mult_now(i,ksn) )
                ELSE
                  dzh_snow_now(i,ksn) = (w_snow_now(i)-sum_weight(i))/REAL(ke_snow-k,vpp) &
                      &                 *rho_w/rho_snow_mult_now(i,ksn)
                ENDIF
              ENDIF
              wtot_snow_now    (i,ksn) = dzh_snow_now(i,ksn)*rho_snow_mult_now(i,ksn)/rho_w
      ...
      ELSE
        ...
        DO i = ivstart, ivend
          IF(rho_snow_now(i) .EQ. 0.0_vpp) rho_snow_now(i) = 250.0_vpp
          IF (l2lay_rho_snow) THEN
            IF(rho_snow_mult_now(i,1) == 0.0_vpp) rho_snow_mult_now(i,1) = rho_snow_now(i)
            IF(rho_snow_mult_now(i,2) == 0.0_vpp) rho_snow_mult_now(i,2) = rho_snow_now(i)
          ENDIF
        END DO
        !$acc end parallel
      END IF
      ...
      DO kso   = 1,ke_soil+1
        ...
        DO i = ivstart, ivend
          IF (t_so_now(i,kso) < (t0_melt-eps_temp)) THEN 
            ...
            zw_m_soil = 0.01_vpp*(zsandf(i)*zd1 + zclayf(i)*zd2 + zsiltf(i)*zd3)
            ...
            IF(zmls(kso) < rootdp(i)) THEN
              ...
              zw_m_low = zporv(i,kso)*zdzhs(kso)*(zzz*zd4 + (1.0_vpp-zzz)*zw_m_soil)
            ELSE
            ...
            w_so_ice_now(i,kso) = MAX (0.0_vpp,w_so_now(i,kso) - zw_m(i))
            w_so_ice_new(i,kso) = MAX (0.0_vpp,w_so_now(i,kso) - zw_m(i))
...
!   -----------------------------------------------------
  ...
  ELSE  ! init_mode > 1, i.e. warmstart (i.e. assimilation cycle and forecast)
    ...
    DO i = ivstart, ivend
      IF (MIN(t_so_now(i,ke_soil+1),t_so_now(i,ke_soil-1)) - t_so_now(i,ke_soil) > 5.0_vpp) THEN
        t_so_now(i,ke_soil) = MIN(t_so_now(i,ke_soil+1),t_so_now(i,ke_soil-1)) - 5.0_vpp
      ENDIF
      IF (t_so_now(i,ke_soil) - MAX(t_so_now(i,ke_soil+1),t_so_now(i,ke_soil-1)) > 5.0_vpp) THEN
        t_so_now(i,ke_soil) = MAX(t_so_now(i,ke_soil+1),t_so_now(i,ke_soil-1)) + 5.0_vpp
      ENDIF
    ENDDO
      ...
      DO kso   = 1,ke_soil+1
        DO i = ivstart, ivend
          IF (t_so_now(i,kso) < (t0_melt-eps_temp)) THEN 
            ...
            wso_ice_equil = MAX (0.0_vpp,w_so_now(i,kso) - zw_m(i))
    ...
    IF(lmulti_snow) THEN ! update of prognostic variables of multi-layer snow model
      ...
      DO i = ivstart, ivend
        IF (h_snow(i) <= eps_soil) THEN  ! snow has disappeared or was already absent
          rho_snow_now(i) = 250.0_vpp
          w_snow_now(i)   = 0.0_vpp

          dzh_snow_now (i,1:ke_snow)     = 0.0_vpp
          wtot_snow_now(i,1:ke_snow)     = 0.0_vpp
          rho_snow_mult_now(i,1:ke_snow) = 0.0_vpp
          wliq_snow_now(i,1:ke_snow)     = 0.0_vpp
        ...
        ELSE IF (h_snow_fg(i) <= eps_soil .AND. h_snow(i) > eps_soil) THEN 
          ...
          rho_snow_now(i) = 250.0_vpp ! needs to be improved
          ...
          dzh_snow_now (i,1:ke_snow) = h_snow(i)/REAL(ke_snow,vpp)
          wtot_snow_now(i,1:ke_snow) = w_snow_now(i)/REAL(ke_snow,vpp)
          wliq_snow_now(i,1:ke_snow) = 0.0_vpp
          t_snow_mult_now(i,1:ke_snow) = t_snow_now(i)
          IF (h_snow(i) > REAL(ke_snow,vpp)*max_toplaydepth) l_redist(i) = .TRUE.
        ELSE IF (h_snow_fg(i) > eps_soil .AND. h_snow_incr(i) < 0.0_vpp) THEN
          ...
          l_redist(i) = .TRUE.
          dz_old(i,1) = MAX(dzh_snow_now (i,1),eps_soil/REAL(ke_snow,vpp))
        ...
        ELSE
          ...
          ! ** snow density should be distinguished between new snow and reestablished old snow **
          ! IF (w_snow_incr(i) > 0.0_vpp) THEN ! assume temperature-dependent fresh snow density - maybe air temp should be used here?!
          !   rho_new(i,1) = MAX(50.0_vpp,125.0_vpp-10.0_vpp*(t0_melt-t_snow_mult_now(i,1)))
          ! ELSE
      ...
      DO ksn = ke_snow,1,-1
        DO i = ivstart, ivend
          IF (l_redist(i)) THEN
            ...
            z_old(i,ksn) = -sum_weight(i) - dzh_snow_now(i,ksn)/2.0_vpp
            sum_weight(i) = sum_weight(i) + dzh_snow_now(i,ksn)
          ENDIF
        END DO
      END DO
        ...
        DO i = ivstart, ivend
          IF (l_redist(i)) THEN
            ...
            IF (ksn == 1) THEN ! Limit top layer to max_toplaydepth
              ...
              zhm_snow(i,ksn) = (-h_snow(i) + zhh_snow(i,ksn))/2.0_vpp
            ...
            ELSE IF (ksn == 2 .AND. ke_snow > 2) THEN ! Limit second layer to 8*max_toplaydepth
              zhh_snow(i,ksn) = MIN( 8.0_vpp*max_toplaydepth+zhh_snow(i,1), zhh_snow(i,1)/(ke_snow-1)*(ke_snow-ksn) )
            ELSE ! distribute the remaining snow equally among the layers
            ...
            IF(dz_old(i,ksn) > 0.0_vpp .AND. rho_snow_mult_now(i,ksn) > 0.0_vpp) THEN
        ...
        DO k = ke_snow,1,-1
          DO i = ivstart, ivend
            IF(l_redist(i)) THEN
              ...
              weight = MAX(MIN(z_old(i,k)+dz_old(i,k)/2.0_vpp,zhm_snow(i,ksn) + dzh_snow_now(i,ksn)/2.0_vpp)-   &
                       MAX(z_old(i,k)-dz_old(i,k)/2.0_vpp, zhm_snow(i,ksn)-dzh_snow_now(i,ksn)/2.0_vpp),0.0_vpp) &
                       &/dzh_snow_now(i,ksn)
      ...
      !
      h_snow      (ivstart:ivend) = 0.0_vpp
      w_snow_now  (ivstart:ivend) = 0.0_vpp
      rho_snow_now(ivstart:ivend) = 0.0_vpp
      t_snow_now  (ivstart:ivend) = 0.0_vpp
...
SUBROUTINE get_wsnow(h_snow, rho_snow, t_snow, istart, iend, soiltyp, w_snow)
  ...
  REAL(vpp), INTENT(INOUT) :: h_snow(:)    ! snow depth            [m]
  REAL(vpp), INTENT(INOUT) :: rho_snow(:)  ! snow density          [kg/m**3]
  REAL(vpp), INTENT(IN)    :: t_snow(:)    ! snow temperature      [K]
  REAL(vpp), INTENT(INOUT) :: w_snow(:)    ! snow water equivalent [m H2O]
  INTEGER,  INTENT(IN)    :: istart       ! start index
  INTEGER,  INTENT(IN)    :: iend         ! end index
  INTEGER,  INTENT(IN)    :: soiltyp(:)   ! type of soil (1-9)

  REAL(vpp) :: rho_w, t0_melt

  ! local
  INTEGER :: jc         ! loop index

  REAL(vpp), PARAMETER :: rho_snw_default=250.0_vpp
  ...
  !
  DO jc = istart, iend
    IF (rho_snow(jc) < 1.0_vpp) rho_snow(jc)=rho_snw_default      ! average initial density
    IF (h_snow(jc) > 0.0_vpp) THEN
      ...
      IF (soiltyp(jc) /= 1) THEN       ! 1=ice
        ...
        h_snow(jc) = MIN(h_snow(jc), 4.0_vpp)
      ...
      ELSE
        ...
        h_snow(jc)   = MAX(h_snow(jc), 1.0_vpp)
        rho_snow(jc) = crhosmax_tmin + MAX(0.0_vpp,(MIN(t0_melt,t_snow(jc))-csnow_tmin)/(t0_melt-csnow_tmin)) &
