...
!  Introducing umfl_s_b, vmfl_s_b, u_s_b, v_s_b (all on staggered positions)
! V5_4e        2017-03-23 Xavier Lapillonne, Katherine Osterried, Ulrich Schaettler
!  Removed fields for the humidity variables qx and their tendencies:
!    These are replaced by data storage for the tracers: trcr_data_b, trcr_data_tens_b
!  Introduced derived type to pass pointer arrays to convection and turbulent diffusion (XL)
!  Introducing fields for SSO scheme in blocked data format (KO)
!  Introduced snow_melt_b for sfc-schemes
!  Renamed variable qvsflx to qvfl_s (in accordance with ICON)
! V5_6         2019-02-27 Ulrich Schaettler
!  Introduced new field for exner pressure epr_b
! V5_6a        2019-05-21 Jan-Peter Schulz
!  Introduce skin temperature approach
! V5_6b        2019-10-16 Pavel Khain
!  Added fields for aerosol climatology, rate of solar and thermal heating (sohr, thhr)
!  Added field weff_rad for CLOUDRAD
! V5_7         2020-02-21 Ulrich Schaettler
!  Added qrt_conv_b, qst_conv_b for treatment in Tiedtke-Bechtold scheme
! v5_7_SNOW    2021-01-08 Varun Sharma
!  Added variables (t_sn_b,theta_i_b,theta_w_b,theta_a_b,dzm_sn_b,hn_sn_b,top_sn_b) for snowpolino
! Code Description:
! Language: Fortran 90.
...
! -----------------------------                           
  ...
  REAL (KIND=wp), ALLOCATABLE, TARGET     ::   &
    ...
    sso_gamma_b  (:),  & ! anisotropy of sub-grid scale orography          --
    sso_theta_b  (:),  & ! angle betw. principal axis of orograpy and E  ( rad )
    sso_sigma_b  (:),  & ! mean slope of sub-grid scale orography          -- 
    aer_su_b     (:),  & ! monthly aerosol climatology sulfate drops     (0 - 1)
    aer_du_b     (:),  & ! monthly aerosol climatology total dust        (0 - 1)
    aer_or_b     (:),  & ! monthly aerosol climatology organic (water sol.)(0 - 1)
    aer_bc_b     (:),  & ! monthly aerosol climatology black carbon      (0 - 1)
    aer_ss_b     (:),  & ! monthly aerosol climatology sea salt          (0 - 1)
    sai_b        (:),  & ! surface area index                              --
    tai_b        (:),  & ! transpiration area index                        --
    eai_b        (:),  & ! (evaporative) earth area index                  --
    skinc_b      (:),  & ! skin conductivity                             (W m-2 K-1)
...
! -----------------------
  REAL (KIND=wp), ALLOCATABLE, TARGET     ::   &
    ...
    u_s_b      (:,:),  & ! zonal wind speed at u-point                   ( m/s )
    v_s_b      (:,:),  & ! meridional wind speed at v-point              ( m/s )
...
! ----------------------------
  REAL (KIND=wp), ALLOCATABLE, TARGET     ::   &
    ...
    t_sk_b     (:),    & ! skin temperature                              (  K  )
    t_sk_new_b (:),    & ! skin temperature at time level nnew           (  K  )
...
! -----------------
  REAL (KIND=wp), ALLOCATABLE, TARGET     ::   &
    ...
    prh_gsp_b  (:)  ,  & ! precipitation rate of hail, grid-scale        (kg/m2*s)
...
! ---------------------------------
  REAL (KIND=wp), ALLOCATABLE, TARGET     ::   &
    ...
    tt_sso_b   (:,:),  & ! t-tendency due to SSO                         (K/s)
    ustr_sso_b (:),    & ! u-stress (surface momentum flux) due to SSO   (N/m2)
    vstr_sso_b (:),    & ! v-stress (surface momentum flux) due to SSO   (N/m2)
    vdis_sso_b (:),    & ! vert. int. dissipation of kin. en. due to SSO (W/m2)
...
! ------------------------
  REAL (KIND=wp), ALLOCATABLE, TARGET     ::   &
    ...
    snow_melt_b        (:)    ! snow melt amount (sum over forecast)
...
! ---------------------------
  ...
  ! Additional fields for the convection interface (and some others)
  REAL (KIND=wp), ALLOCATABLE, TARGET     ::   &
    fis_b            (:), & !
    fif_b          (:,:), & !
    fih_b          (:,:), & !
    ...
    qhfl_b           (:), & ! (averaged) qhfl (qvfl_s) for convection
...
!   fields that are computed in the dynamics and / or physics
  REAL (KIND=wp), ALLOCATABLE, TARGET     ::   &
    ...
    qvfl_s_b         (:), & ! surface flux of water vapour                  (kg/m2s)
    umfl_s_b         (:), & ! u-momentum flux (surface)                     ( N/m2)
    vmfl_s_b         (:)    ! v-momentum flux (surface)                     ( N/m2)
...
! 8. Derived type to pass pointer arrays to convection and turbulent diffusion
! ----------------------------------------------------------------------------
...
!VS <
! 9. fields for the new multi_layer snow scheme (SNOWPOLINO)
! ----------------------------------------------------------------------------

  REAL (KIND=wp), ALLOCATABLE, TARGET     ::   &

    t_sn_b           (:,:) , & ! multi-layer snow temperature               ( K )
    t_sn_new_b       (:,:) , & !

    theta_i_b        (:,:) , & ! volumentric ice content                    ( - )
    theta_i_new_b    (:,:) , &

    theta_w_b        (:,:) , & ! volumentric water content                  ( - )
    theta_w_new_b    (:,:) , &

    theta_a_b        (:,:) , & ! volumentric air content                    ( - )
    theta_a_new_b    (:,:) , &

    dzm_sn_b         (:,:) , & ! layer thickness                            ( m )
    dzm_sn_new_b     (:,:) , &

    hn_sn_b          (:)   , & ! new snow amounts (storage)                 ( m )
    hn_sn_new_b      (:)   , &

    top_sn_b         (:)   , & ! index of first (top) layer
    top_sn_new_b     (:)
!VS >
