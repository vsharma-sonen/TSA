...
USE data_fields,           ONLY:                                            &
    ...
    u_bd, t_bd,                                                             &
!VS <
    h_snow,theta_i,theta_w,theta_a,dzm_sn,t_sn,top_sn
!VS >
...
!----------------------------------------------------------------------
  ...
  ! Grib variables
!DL  INTEGER (KIND=intgribf),  PARAMETER  :: &
!DL       npds   =    321_intgribf         , & ! Dimension for product definition section (pds)
!DL       ngds   =    626_intgribf         , & ! Dimension for grid description section (gds)
!DL       nbms   =      3_intgribf         , & ! Dimension for bit map section (bms)
!DL       nbds   =     11_intgribf         , & ! Dimension for binary data section
!DL       ndsup  =     73_intgribf         , & ! Dimension for dsup
!DL       ndims  =     20_intgribf             ! Dimension for idims (contains all dimensions)
...
!DL  INTEGER (KIND=intgribf)              :: &
!DL       iz_ps=1                          , & ! 
!DL       ierrf                                ! 

  ! The following dimensions are set during program execution
  INTEGER (KIND=intgribf)              :: &
!DL       lfd                              , & ! Dimension for iblock
!DL       lbm                              , & ! Dimension for bitmap: this value has to be at
                                            !   least the same as lbmax in subroutine grbin1
       lds                                  ! Dimension for unpacked data

!DL  INTEGER (KIND=intgribf)              :: &
!DL       idims (ndims)                    , & ! array for all dimensions (output)
!DL       ipds  (npds)                     , & ! product description section
!DL       igds  (ngds)                     , & ! grid description section
!DL       ibms  (nbms)                     , & ! bit map section
!DL       ibds  (nbds)                         ! binary data section

  ! Arrays that are allocated during program execution
!DL  INTEGER (KIND=intgribf), ALLOCATABLE :: &
!DL       iblock    (:)                    , & ! array for gribed data
!DL       ibmap     (:)                        ! array for bit map

  REAL   (KIND=irealgrib), ALLOCATABLE :: &
!DL       dsup (:)                         , & ! array for special data
       ds   (:)                         , & ! array for unpacked data
       ds2(:,:)                             !
  ...
  INTEGER (KIND=intgribf)              :: &
       ilen                             , & ! 
       ierr                             , & ! 
!DL New for negative startStep (GRIB2)
       ifac                             , & !
       istat                                ! 
     INTEGER  (KIND=intgribc)             :: & ! corresponding variables for C-routines
          nudatc                           , & ! 
...
!DL     IF (iednr == 1) CALL grib_set (igribo, 'stepType','accum')
        IF (iednr == 1) CALL grib_set (igribo, 'timeRangeIndicator',4)
        CALL grib_set (igribo, 'typeOfLevel', 'surface')
        CALL grib_set (igribo, 'endStep',0)
        CALL grib_set (igribo, 'startStep',  int(zfac * dt * real(nout_interval,wp)))
...
!GDM>  store  
     ELSE IF (i==2*(ke_soil+1)+18) THEN
        ...
        IF (lcheck) THEN
           CALL stats("rstom",store(:,:,1),llandmask,UNIT=444)
        ENDIF
     ELSE IF (i==3*(ke_soil+1)+19) THEN
        CALL grib_set (igribo, 'shortName', 'T_2M')
        ds2=REAL(t_2m,irealgrib)         
        IF (lcheck) THEN
           CALL stats("t_2m",t_2m,llandmask,UNIT=444)
        ENDIF
     ELSE IF (i==3*(ke_soil+1)+20) THEN
     ...
     ELSE IF (i==3*(ke_soil+1)+24) THEN
        ...
        ENDIF
     ELSE IF (i==3*(ke_soil+1)+25) THEN
        CALL grib_set (igribo, 'shortName', 'H_SNOW')
        CALL grib_set (igribo, 'typeOfLevel', 'surface')
        ds2=REAL(h_snow(:,:,nnew),irealgrib)   
        IF (lcheck) THEN
          CALL stats("t_g",h_snow(:,:,nnew),llandmask,UNIT=444)
        ENDIF
     ENDIF

     ! undef handling for non-landpoints
!DL change for test
!!DL  rmin=MINVAL(ds2,llandmask)
!!DL WHERE (.NOT.llandmask)
!DL     ds2(:,:)=undefgrib !rmin
!!DL    ds2(:,:)=rmin
!!DL END WHERE
...
!DL  CALL grbex1(iednr, iz_ps, undefgrib, ndims, idims, &                  ! Coding to GRIB
!DL       ipds, igds, ibms, ibds, ibmap, dsup , ds, iblock, ierrf)
!DL  ilenc=idims(18)*iwlength
!DL  CALL cuegex (nudatc, iblock, ilenc, ierrc)                            ! Write to file
  ...
  if(lsnow) then
     ...
     ! output air in snow
     do i=1,ke_snow    
       call grib_set (igribo, 'productDefinitionTemplateNumber',40)
       CALL grib_set (igribo, 'shortName', 'AIRinSNW_VC_M')
       CALL grib_set (igribo, 'typeOfLevel', 'snowLayer')
       CALL grib_set (igribo, 'topLevel',   INT(i  ,intgribf) )
       CALL grib_set (igribo, 'bottomLevel',INT(i-1,intgribf) )
       
       ds2=REAL(theta_a(:,:,i,nnew),irealgrib)   
       ds=RESHAPE(ds2,(/ idim*jdim /))
       call grib_set(igribo, 'values', ds(1:lds), ierr)
       if( ierr /= GRIB_SUCCESS) then
          WRITE(6,*) "Error in grib_set (values) for field ",i," of processed fields"
     ...
     do i=1,ke_snow    
       call grib_set (igribo, 'productDefinitionTemplateNumber',40)
       CALL grib_set (igribo, 'shortName', 'ICEinSNW_VC_M')
       CALL grib_set (igribo, 'typeOfLevel', 'snowLayer')
       CALL grib_set (igribo, 'topLevel',   INT(i  ,intgribf) )
       CALL grib_set (igribo, 'bottomLevel',INT(i-1,intgribf) )
       
       
       ds2=REAL(theta_i(:,:,i,nnew),irealgrib)   
       ds=RESHAPE(ds2,(/ idim*jdim /))
       call grib_set(igribo, 'values', ds(1:lds), ierr)
       if( ierr /= GRIB_SUCCESS) then
          WRITE(6,*) "Error in grib_set (values) for field ",i," of processed fields"
     ...
     do i=1,ke_snow    
       call grib_set (igribo, 'productDefinitionTemplateNumber',40)
       CALL grib_set (igribo, 'shortName', 'H2OinSNW_VC_M')
       CALL grib_set (igribo, 'typeOfLevel', 'snowLayer')
       CALL grib_set (igribo, 'topLevel',   INT(i  ,intgribf) )
       CALL grib_set (igribo, 'bottomLevel',INT(i-1,intgribf) )
       
       
       ds2=REAL(theta_w(:,:,i,nnew),irealgrib)   
       ds=RESHAPE(ds2,(/ idim*jdim /))
       call grib_set(igribo, 'values', ds(1:lds), ierr)
       if( ierr /= GRIB_SUCCESS) then
          WRITE(6,*) "Error in grib_set (values) for field ",i," of processed fields"
     ...
     do i=1,ke_snow    
       call grib_set (igribo, 'productDefinitionTemplateNumber',40)
       CALL grib_set (igribo, 'shortName', 'DZ_SNOW_M')
       CALL grib_set (igribo, 'typeOfLevel', 'snowLayer')
       CALL grib_set (igribo, 'topLevel',   INT(i  ,intgribf) )
       CALL grib_set (igribo, 'bottomLevel',INT(i-1,intgribf) )
       
       
       ds2=REAL(dzm_sn(:,:,i,nnew),irealgrib)   
       ds=RESHAPE(ds2,(/ idim*jdim /))
       call grib_set(igribo, 'values', ds(1:lds), ierr)
       if( ierr /= GRIB_SUCCESS) then
          WRITE(6,*) "Error in grib_set (values) for field ",i," of processed fields"
     ...
     do i=1,ke_snow    
       call grib_set (igribo, 'productDefinitionTemplateNumber',40)
       CALL grib_set (igribo, 'shortName', 'T_SNOW_M')
       CALL grib_set (igribo, 'typeOfLevel', 'snowLayer')
       CALL grib_set (igribo, 'topLevel',   INT(i  ,intgribf) )
       CALL grib_set (igribo, 'bottomLevel',INT(i-1,intgribf) )
       
       
       ds2=REAL(t_sn(:,:,i,nnew),irealgrib)   
       ds=RESHAPE(ds2,(/ idim*jdim /))
       call grib_set(igribo, 'values', ds(1:lds), ierr)
       if( ierr /= GRIB_SUCCESS) then
          WRITE(6,*) "Error in grib_set (values) for field ",i," of processed fields"
     ...
     call grib_set (igribo, 'productDefinitionTemplateNumber',40)
     CALL grib_set (igribo, 'shortName', 'N_SNOW_M')
     CALL grib_set (igribo, 'typeOfLevel', 'surface')
     CALL grib_set (igribo, 'topLevel',   INT(i  ,intgribf) )
     CALL grib_set (igribo, 'bottomLevel',INT(i-1,intgribf) )
     
     
     ds2=REAL(top_sn(:,:,nnew),irealgrib)   
     ds=RESHAPE(ds2,(/ idim*jdim /))
     call grib_set(igribo, 'values', ds(1:lds), ierr)
     if( ierr /= GRIB_SUCCESS) then
        WRITE(6,*) "Error in grib_set (values) for field ",i," of processed fields"
