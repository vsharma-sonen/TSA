...
!  This is meteo_utilities from COSMO version 5.07 without modifications
!
!==============================================================================
!
! Description:
...
!  Corrected comments
! V5_5         2018-02-23 Valentin Clement, Katherine Silverthorne Osterried,
!                         Xavier Lapillonne
!  Ported subroutine satad to GPU with OpenACC (VC)
!  Introduced scalar subroutine satad_s for GPU port of microphysics (KSO)
!  Added OpenACC statment for calrho_block and calps_block (XL)
! V5_5a        2018-06-22 Tobias Wicki, Xavier Lapillonne, Ulrich Schaettler
!  Ported the computation of diagnostics to GPU if lacc is .TRUE. (TW, XL)
!  Removed all iintegers (US)
!  Use kind_parameters instead of data_parameters (US)
! V5_7         2020-02-21 Yannick Boetzel
...
LOGICAL, INTENT(IN), OPTIONAL :: lacc !this should be set to true
                                      ...
                                      !data region where in and out variable
                                      !are present
! Local variable:
LOGICAL :: lzacc
INTEGER :: i,j,k
...
SUBROUTINE calps ( ps, pp, t, qv, qc, qrs, rho0, p0, dp0,          &
                   ie, je, rvd_m_o, r_d, istart, iend, jstart, jend, lacc)
...
!   hydrostatic manner.
!
!   The fields are passed two-dimensional for the ij-data structure.
!   The calling procedure has to choose the proper level and time level.
!
! Method:
...
Do j = jstart, jend
  Do i = istart, iend
    ps(i,j) = ( p0(i,j) + pp(i,j) )*EXP( 0.5_wp*dp0(i,j) / (t(i,j)*(1.0_wp + rvd_m_o*qv(i,j) - qc(i,j) - qrs(i,j))* r_d * rho0(i,j)))
  ENDDO
ENDDO
!$acc end parallel
...
!   hydrostatic manner.
!
!   The fields are passed one-dimensional for the blocked data structure. 
!   The calling procedure has to choose the proper level and time level.
!
! Method:
!   Formula to be described in the documentation
...
INTEGER, INTENT (IN) :: &  ! dimensions and run indices
...
SUBROUTINE SATAD ( kitera, te, qve, qce, tstart, phfe,                        &
                   ...
                   cp_d, idim, jdim, ilo, iup, jlo, jup, lacc )
...
! STATEMENT FUNCTIONS
  ...
  ! Initialize the local switch to run the subroutine on GPU.
  IF (PRESENT(lacc)) THEN
    lzacc = lacc
  ELSE
    lzacc = .FALSE.
...
!==============================================================================
!Scalar version of SATAD, operatates on a single grid point 
SUBROUTINE satad_s(kitera, te, qve, qce, tstart, phfe,                        &
                   zdqd  , zqdwe, zh   , ztg0  , ztgn, zdqdt0, zgqd0, zphe ,  &
                   b1, b2w, b3, b4w, b234w, rdrd, emrdrd, rddrm1, lh_v, cpdr, &
                   cp_d )
!-------------------------------------------------------------------------------
...
!
! Method:
!   Saturation adjustment, i.e. reversible condensation/evaporation at constant
!   pressure by assuming chemical equilibrium of water and vapor
!
!-------------------------------------------------------------------------------

!$acc routine seq

!Routine can be called for a kernel
...
! --------------------
  ...
  REAL (KIND=wp), INTENT (IN) ::  &
    tstart, & ! Start temperature for iteration
    phfe      ! Pressure (input)     
...
! assume first subsaturation
  ...
  qce  = 0.0_wp
  zgeu      = fgew(te)
  zgqdu     = fgqd(zgeu,phfe)
  zdqd = zgqdu - zqdwe

! Do saturation adjustments if current gripoint saturated 
! ------------------------------------------------------


!XL: replaced returned statment with if block
IF (zdqd < 0.0_wp ) THEN

  zh = cp_d*te + lh_v*qve
  zphe = phfe
  zgew       = fgew(ztg0)
  zgqd0 = fgqd(zgew,zphe)


  IF ( kitera > 1 ) THEN
    DO  nzit  = 1 , kitera-1
        zdqdt0= fdqdt(ztg0,zgqd0)
        ztg0  = (zh - lh_v*(zgqd0-zdqdt0*ztg0))/ &
                      ( cp_d + lh_v*zdqdt0)
        zgew        = fgew(ztg0)
        zgqd0  = fgqd(zgew,zphe)
    ENDDO
...
!-------------------------------------------------------------------------------
  ...
  zdqdt0 = fdqdt(ztg0,zgqd0)
  ztgn  = ( zh - lh_v*(zgqd0-zdqdt0*ztg0) ) / &
          ( cp_d + lh_v*zdqdt0 )
  zgqd0 = zgqd0 + zdqdt0*( ztgn-ztg0 )
...
! -----------------------------------
      ...
      te =  ztgn
      qve = zgqd0
      qce = MAX( zqdwe - zgqd0, zqwmin )
    END IF

! End of the subroutine

END SUBROUTINE SATAD_S
...
LOGICAL, INTENT(IN), OPTIONAL :: lacc !this should be set to true
                                      !if the function is called within
                                      !data region where in and out variables
                                      !are present
...
! Begin subroutine tgcom
  ...
  !$acc parallel default(present) vector_length(64) if (lzacc)
  !$acc loop gang
  DO j = jstart, jend
    !$acc loop vector
    DO i = istart, iend
      IF (llp(i,j) .AND. (ws(i,j) > 0.0_wp)) THEN
        tg(i,j) = ts(i,j) + (1.0_wp - MIN(1.0_wp,ws(i,j)/cf_snow)) &
             * (tb(i,j) - ts(i,j))
      ELSE
        tg(i,j) =   tb(i,j)
      ENDIF
